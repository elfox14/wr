...
function isFinishedMatch(match: any) { return String(match?.status || match?.displayStatus || '').toUpperCase() === 'FINISHED'; }
...
function isDebutTeam(team: any) { return Boolean(team?.isWorldCupDebut || team?.worldCupDebut || team?.firstWorldCup || team?.debutWorldCup || team?.firstAppearance); }
...
export default function HomeClientSportsNext({ initialAssets, upcomingMatches = [], assetsCount = 0, playersCount = 0, teamsCount = 0, upcomingMatchesCount = 0, academyArticles = [] }: Props) {
  ...
  const matchCard = (match: any, options?: { title?: string; compact?: boolean; countdown?: boolean }) => {
    ...
    return <div className={`rounded-2xl border p-4 ${live ? 'border-emerald-400/25 bg-emerald-400/[0.06]' : finished ? 'border-gray-500/20 bg-gray-500/[0.04]' : 'border-orange-400/20 bg-orange-400/[0.04]'}`}>
  ...
  return <main className="mx-auto max-w-7xl space-y-4 px-4 py-4 sm:px-6 sm:py-5 lg:space-y-5 lg:px-8">
    <section className="overflow-hidden rounded-[1.2rem] border border-white/10 bg-black/40"><div className="flex flex-col gap-2 px-4 py-2 lg:flex-row lg:items-center lg:justify-between"><div className="flex flex-wrap items-center gap-2"><span className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-black ${hasLive ? 'border-emerald-400/30 bg-emerald-400/10 text-emerald-300' : 'border-[#0FF0FC]/25 bg-[#0FF0FC]/10 text-[#0FF0FC]'}`}><span className={`h-2 w-2 rounded-full ${hasLive ? 'animate-pulse bg-emerald-400' : 'bg-[#0FF0FC]'}`} />{hasLive ? 'مباشر الآن' : 'مباريات اليوم'}</span><div className="flex flex-wrap items-center gap-2 text-sm font-bold text-gray-300">{currentMatch ? <><span className="text-white">{currentMatch.homeTeam?.name || 'الفريق الأول'}</span><span className="rounded-lg border border-white/15 bg-black/30 px-3 py-1 font-mono font-black text-[#FFD700] tabular-nums">{currentIsLive || String(currentMatch.status).toUpperCase() === 'FINISHED' ? `${currentMatch.homeScore ?? 0} - ${currentMatch.awayScore ?? 0}` : 'VS'}</span><span className="text-white">{currentMatch.awayTeam?.name || 'الفريق الثاني'}</span><Link href={animationHref(currentMatch)} className="rounded-full border border-[#FFD700]/30 bg-[#FFD700]/10 px-3 py-1 text-xs font-black text-[#FFD700] transition hover:bg-[#FFD700] hover:text-black">متابعة المباشر</Link></> : 'تابع المباريات والأخبار والتحليلات من مكان واحد'}</div></div><div className="hidden min-w-0 items-center justify-end lg:flex">{stripMatch ? <Link href="/matches" className="inline-flex max-w-full items-center gap-2 rounded-full border border-[#0FF0FC]/20 bg-[#0FF0FC]/10 px-3 py-1.5 text-xs font-black text-white transition hover:bg-[#0FF0FC]/15"><span className="shrink-0 text-[#0FF0FC]">المباراة القادمة</span><span className="truncate">{stripMatch.homeTeam?.name || 'الفريق الأول'}</span><span className="rounded-md bg-black/40 px-2 py-0.5 text-[#FFD700]">VS</span><span className="truncate">{stripMatch.awayTeam?.name || 'الفريق الثاني'}</span><Clock size={12} className="shrink-0 text-gray-400" /><span className="shrink-0 text-gray-300">{isUpcoming(stripMatch, now) ? countdown(stripMatch, now) : statusLabel(stripMatch, now)}</span></Link> : <Link href="/matches" className="inline-flex items-center gap-2 rounded-full border border-[#0FF0FC]/20 bg-[#0FF0FC]/10 px-3 py-1.5 text-xs font-black text-[#0FF0FC]">عرض المباريات <ArrowLeft size={13} /></Link>}</div></div></section>
    ...
    <section className="relative overflow-hidden rounded-[1.45rem] border border-white/10 bg-[radial-gradient(circle_at_top_right,rgba(15,240,252,0.18),transparent_28%),radial-gradient(circle_at_bottom_left,rgba(255,215,0,0.12),transparent_24%),linear-gradient(135deg,rgba(255,255,255,0.055),rgba(255,255,255,0.015))] p-3 shadow-anti-gravity sm:p-4 md:p-5"><div className="pointer-events-none absolute inset-0 opacity-16 [background-image:linear-gradient(rgba(255,255,255,.05)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.05)_1px,transparent_1px)] [background-size:44px_44px]" /><div className="relative grid gap-4 lg:grid-cols-[0.96fr_1.04fr] lg:items-start"><div className="flex flex-col justify-start pt-1"><div className="mb-2 flex flex-wrap items-center gap-2"><p className="inline-flex w-fit items-center gap-2 rounded-full border border-[#0FF0FC]/25 bg-[#0FF0FC]/10 px-3 py-1 text-[10px] font-black text-[#0FF0FC]"><Sparkles size={13} /> كأس العالم مع MC PRIME</p><span className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-[10px] font-black ${heroStatus.cls}`}><span className={`h-2 w-2 rounded-full ${heroStatus.dot}`} />{heroStatus.label}</span></div><h1 className="max-w-3xl text-xl font-black leading-tight text-white sm:text-2xl md:text-3xl">كل ما يحدث في كأس العالم… مباشر، موثق، وتحليلي</h1><p className="mt-2 max-w-2xl text-[11px] leading-5 text-gray-300 sm:text-xs md:text-sm">ابدأ من المباراة والنتيجة والخبر، ثم انتقل للتحليل الكروي وصفحات المنتخبات. البورصة طبقة ترفيهية افتراضية في النهاية.</p><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">{heroStats.map((stat) => { const Icon = stat.icon; return <div key={stat.label} className="rounded-xl border border-[#FFD700]/20 bg-black/30 p-2.5"><Icon className="mb-1.5 text-[#FFD700]" size={16} /><div className="text-lg font-black text-white sm:text-xl">{arNumber.format(stat.value)}</div><div className="text-[10px] font-bold text-gray-400">{stat.label}</div></div>; })}</div><div className="mt-3 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap"><Link href="/matches" className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#0FF0FC] px-3 py-2 text-xs font-black text-black transition hover:bg-[#70f7ff]"><Radio size={16} /> مركز المباريات</Link><Link href="/articles" className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/10 px-3 py-2 text-xs font-black text-white transition hover:bg-white/15"><Newspaper size={16} /> الأخبار والتحليل</Link></div></div><div className="rounded-[1.25rem] border border-white/10 bg-black/40 p-3 backdrop-blur-xl sm:p-4"><div className="mb-3 flex items-center justify-between gap-3"><div><p className="text-[11px] font-black text-gray-500">{hasLive ? 'مباراة جارية الآن' : 'مباراتان من اليوم'}</p><h2 className="mt-1 text-base font-black text-white sm:text-lg">مباريات اليوم</h2></div><ShieldCheck className={hasLive ? 'text-emerald-300' : 'text-[#0FF0FC]'} size={26} /></div>{currentMatch ? <div>{matchCard(currentMatch)}{secondMatch ? <div className="mt-3">{matchCard(secondMatch, { title: currentIsLive ? 'المباراة التالية' : 'المباراة الثانية اليوم', compact: true, countdown: isUpcoming(secondMatch, now) })}<Link href="/matches" className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#0FF0FC] px-3 py-2 text-xs font-black text-black transition hover:bg-[#70f7ff]">عرض صفحة المباريات <ArrowLeft size={14} /></Link></div> : <div className="mt-3 rounded-2xl border border-[#0FF0FC]/18 bg-black/35 p-3"><p className="text-[10px] font-black text-[#0FF0FC]">المباراة الثانية</p><h3 className="mt-1 text-sm font-black text-white">لا توجد مباراة ثانية محملة حاليًا</h3><Link href="/matches" className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#0FF0FC] px-3 py-2 text-xs font-black text-black">عرض صفحة المباريات <ArrowLeft size={14} /></Link></div>}</div> : <div className="rounded-2xl border border-dashed border-white/10 p-6 text-center text-sm text-gray-500">لم يتم تحميل مباريات بعد.<Link href="/matches" className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-[#0FF0FC]/25 bg-[#0FF0FC]/10 px-3 py-2 text-xs font-black text-[#0FF0FC]">فتح صفحة المباريات <ArrowLeft size={14} /></Link></div>}</div></div></section>
    <section className="grid gap-4 lg:grid-cols-[0.95fr_1.05fr]"><div id="news" className="scroll-mt-24 rounded-[1.6rem] border border-white/10 bg-white/[0.035] p-4 sm:p-5"><div className="mb-4 flex items-center justify-between"><div><p className="text-xs font-black text-[#FFD700]">News Room</p><h2 className="mt-1 text-xl font-black text-white sm:text-2xl">آخر الأخبار والتحليلات</h2></div><Newspaper className="text-[#FFD700]" size={26} /></div>{academyArticles.length ? <div className="space-y-3">{academyArticles.slice(0, 3).map((article) => <Link key={article.id} href={`/article/${article.id}`} className="block rounded-2xl border border-white/8 bg-black/25 p-4 transition hover:border-[#FFD700]/25 hover:bg-white/[0.055]"><div className="mb-2 flex items-center justify-between gap-3"><span className="rounded-full bg-[#FFD700]/10 px-3 py-1 text-[10px] font-black text-[#FFD700]">{article.category}</span>{article.readingTime ? <span className="text-[10px] font-bold text-gray-500">{article.readingTime}</span> : null}</div><h3 className="line-clamp-2 font-black text-white">{article.title}</h3><p className="mt-2 line-clamp-2 text-sm leading-6 text-gray-400">{article.excerpt}</p></Link>)}</div> : <div className="rounded-2xl border border-dashed border-white/10 p-6 text-center text-sm leading-7 text-gray-500">لا توجد أخبار موثقة منشورة بعد. عند إضافة مقالات أو تقارير ستظهر هنا تلقائيًا.</div>}</div><div id="analysis" className="scroll-mt-24 rounded-[1.6rem] border border-[#0FF0FC]/20 bg-[linear-gradient(135deg,rgba(15,240,252,0.10),rgba(255,255,255,0.025))] p-4 shadow-[0_0_35px_rgba(15,240,252,0.06)] sm:p-5"><div className="mb-4 flex items-center justify-between"><div><p className="text-xs font-black text-[#0FF0FC]">Analysis Desk</p><h2 className="mt-1 flex items-center gap-2 text-xl font-black text-white sm:text-2xl"><LineChart size={22} className="text-[#0FF0FC]" /> مركز التحليل</h2></div><span className="rounded-full border border-[#0FF0FC]/20 bg-[#0FF0FC]/10 px-3 py-1 text-[10px] font-black text-[#0FF0FC]">رأي تحليلي منفصل عن الخبر</span></div><div className="grid gap-3 md:grid-cols-3">{analysisCards.map((card) => { const Icon = card.icon; return <Link key={card.title} href={card.href} className="rounded-2xl border border-[#0FF0FC]/12 bg-black/25 p-4 transition hover:border-[#0FF0FC]/35 hover:bg-[#0FF0FC]/[0.06]"><Icon className="mb-3 text-[#0FF0FC]" size={23} /><h3 className="font-black text-white">{card.title}</h3><p className="mt-2 text-xs leading-6 text-gray-400">{card.text}</p></Link>; })}</div><div className="mt-4 rounded-2xl border border-white/10 bg-black/25 p-4"><div className="mb-3 flex items-center justify-between gap-3"><div><p className="text-[11px] font-black text-[#0FF0FC]">Football Spotlight</p><h3 className="text-sm font-black text-white">تحليل كروي مختصر لمنتخبات مختارة عشوائيًا</h3></div><span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[10px] font-black text-gray-400">يتغير مع تحديث الصفحة</span></div><div className="grid gap-3 sm:grid-cols-2">{(spotlightTeams.length ? spotlightTeams : teams).slice(0, 4).map((team) => <Link key={team.id || team.name} href={team?.id ? `/asset/${team.id}` : '/market?type=TEAM'} className="flex gap-3 rounded-2xl border border-white/8 bg-black/25 p-3 transition hover:border-[#0FF0FC]/25 hover:bg-white/[0.055]"><AssetImage image={team?.image} name={team?.name || 'منتخب'} type="TEAM" width={42} height={42} className="h-11 w-11 shrink-0 rounded-full border border-white/10 bg-black/40 object-cover" /><div className="min-w-0"><div className="truncate text-sm font-black text-white">{team?.name || 'منتخب'}</div><p className="mt-1 text-[11px] leading-5 text-gray-400">{footballSummary(team)}</p></div></Link>)}</div></div></div></section>
    {debutTeams.length > 0 ? <section className="rounded-[1.5rem] border border-emerald-400/20 bg-emerald-400/[0.045] p-4 sm:p-5"><div className="mb-4 flex items-center justify-between"><div><p className="text-xs font-black text-emerald-300">Discovery</p><h2 className="mt-1 text-xl font-black text-white sm:text-2xl">منتخبات تشارك لأول مرة</h2></div><Trophy className="text-emerald-300" size={26} /></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{debutTeams.map((team) => <Link key={team.id || team.name} href={team?.id ? `/asset/${team.id}` : '/market?type=TEAM'} className="rounded-2xl border border-white/8 bg-black/25 p-4 transition hover:border-emerald-300/35 hover:bg-emerald-300/[0.06]"><AssetImage image={team?.image} name={team?.name || 'منتخب'} type="TEAM" width={46} height={46} className="mb-3 h-12 w-12 rounded-full border border-white/10 bg-black/40 object-cover" /><h3 className="font-black text-white">{team?.name || 'منتخب'}</h3><p className="mt-2 text-xs leading-6 text-gray-400">ظهور أول موثق داخل بيانات المنصة. غير ذلك لا يتم عرض هذا القسم.</p></Link>)}</div></section> : null}
