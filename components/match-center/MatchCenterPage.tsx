...
function sideLineup(lineup: Record<string, any>, side: 'home' | 'away') {
...
function lineupPlayers(lineup: Record<string, any>, squad: any[]) {
  ...
  if (official.length) return official.slice(0, 11);
  return squad.slice(0, 11).map((player) => ({ name: player.name, image: player.image, number: player.code }));
}

function usedSubs(lineup: Record<string, any>) {
  return arr(lineup.usedSubstitutes, lineup.substitutesUsed, lineup.substitutedIn, lineup.used_substitutes, lineup.substitutions).map(parsePlayer).filter(Boolean) as PlayerCard[];
}
...
function PlayerDot({ player, color }: { player: PlayerCard; color: string }) {
  return (
    <div className="flex min-w-0 flex-col items-center gap-1">
      <div className={`relative flex h-11 w-11 items-center justify-center overflow-hidden rounded-full border bg-black/60 ${color}`}>
        {player.image ? <img src={player.image} alt={player.name} className="h-full w-full object-cover" loading="lazy" /> : <span className="text-xs font-black text-white">{initials(player.name)}</span>}
        {player.number ? <span className="absolute -bottom-0.5 -left-0.5 rounded-full bg-[#FFD700] px-1 text-[9px] font-black text-black">{player.number}</span> : null}
      </div>
      <p className="max-w-[56px] truncate text-[9px] font-black text-white">{player.name}</p>
    </div>
  );
}
...
function Pitch({ players, formation, side }: { players: PlayerCard[]; formation?: string | null; side: 'home' | 'away' }) {
  ...
  return (
    <div className="relative h-[330px] overflow-hidden rounded-2xl border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,.08),rgba(255,255,255,.02))] p-3">
      <div className="absolute inset-3 rounded-xl border border-white/15" />
      <div className="absolute left-1/2 top-3 h-[calc(100%-24px)] w-px bg-white/10" />
      <div className="absolute left-1/2 top-1/2 h-14 w-14 -translate-x-1/2 -translate-y-1/2 rounded-full border border-white/10" />
      <div className="relative z-10 flex h-full flex-col-reverse justify-between">
        {lines.map((line, index) => <div key={index} className="flex items-start justify-around gap-1">{line.map((player, playerIndex) => <PlayerDot key={`${player.name}-${playerIndex}`} player={player} color={color} />)}</div>)}
      </div>
    </div>
  );
}

function FlagBadge({ team, side }: { team: any; side: 'home' | 'away' }) {
...
function StatRow({ label, pair, suffix = '' }: { label: string; pair: Pair; suffix?: string }) {
  const width = share(pair);
  return <div className="grid grid-cols-[52px_1fr_100px_1fr_52px] items-center gap-2 border-b border-white/10 py-2.5 last:border-b-0 sm:grid-cols-[70px_1fr_190px_1fr_70px] sm:gap-4"><b className="text-center text-lg text-white tabular-nums sm:text-2xl">{fmt(pair?.home, suffix)}</b><div className="h-2.5 overflow-hidden rounded-full bg-white/10"><div className="ml-auto h-full rounded-full bg-gradient-to-l from-[#0FF0FC] to-[#69d7ff]" style={{ width: `${width.home}%` }} /></div><div className="min-h-10 rounded-xl border border-white/10 bg-black/60 px-2 py-2 text-center text-[11px] font-black text-white sm:text-sm">{label}</div><div className="h-2.5 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-gradient-to-r from-[#FFD700] to-[#ffea83]" style={{ width: `${width.away}%` }} /></div><b className="text-center text-lg text-white tabular-nums sm:text-2xl">{fmt(pair?.away, suffix)}</b></div>;
}

function AdvancedCard({ label, description, pair, decimal = false }: { label: string; description: string; pair: Pair; decimal?: boolean }) {
  return <div className="rounded-2xl border border-[#FFD700]/20 bg-[#FFD700]/[.055] p-4 text-center"><p className="text-lg font-black text-white">{description}</p><p className="mt-1 text-xs font-black text-[#FFD700]">{label}</p><div className="mt-3 grid grid-cols-[1fr_auto_1fr] items-center gap-3"><b className="text-3xl text-[#69d7ff]">{decimal ? dec(pair?.home) : fmt(pair?.home)}</b><span className="rounded-xl border border-white/10 bg-black/35 px-3 py-2 text-xs font-black text-white">{label}</span><b className="text-3xl text-[#FFD700]">{decimal ? dec(pair?.away) : fmt(pair?.away)}</b></div></div>;
}

function LineupPanel({ team, lineup, squad, side }: { team: any; lineup: Record<string, any>; squad: any[]; side: 'home' | 'away' }) {
  const formation = String(lineup.formation || lineup.shape || '') || null;
  const players = lineupPlayers(lineup, squad);
  const substitutes = usedSubs(lineup);
  const tone = side === 'home' ? 'border-[#0FF0FC]/20 bg-[#0FF0FC]/[.04] text-[#69d7ff]' : 'border-[#ff4d5e]/20 bg-[#ff4d5e]/[.04] text-[#ff858f]';
  return <div className={`rounded-2xl border p-3 ${tone}`}><div className="mb-3 flex items-center justify-between gap-3"><div><h3 className="text-xl font-black text-white">{teamName(team, 'الفريق')}</h3><p className="text-xs font-bold text-gray-400">١١ لاعبًا أساسيًا</p></div><b className="rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-2xl">{formation || '—'}</b></div><Pitch players={players} formation={formation} side={side} /><div className="mt-3 rounded-2xl border border-white/10 bg-black/25 p-3"><div className="mb-2 flex items-center justify-between"><p className="text-sm font-black text-white">البدلاء المشاركون</p><span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-black">{fmt(substitutes.length)}</span></div>{substitutes.length ? <div className="flex flex-wrap gap-2">{substitutes.map((player) => <PlayerDot key={player.name} player={player} color={side === 'home' ? 'border-[#0FF0FC]/70' : 'border-[#ff5a67]/70'} />)}</div> : <p className="text-xs font-bold text-gray-500">غير متوفر من بيانات التبديلات الرسمية.</p>}</div></div>;
}
...
export default async function MatchCenterPage({ params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  const rows = [
    ...
    ['تسديدات خارج المرمى', pairFromDerived(obj(theStats?.rawData).derived?.shotsOffTargetForLocalCompare) || pair(baseSnapshot, stats, 'shotsOffTarget', 'homeShotsOffTarget', 'awayShotsOffTarget'), ''],
