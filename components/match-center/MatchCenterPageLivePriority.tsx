...
function minuteFrom(match: any, sources: any[]) {
  ...
  if (LIVE.includes(String(match.status || '').toUpperCase()) && Number.isFinite(startMs)) return Math.max(1, Math.min(130, Math.floor((Date.now() - startMs) / 60000) + 1));
  return null;
}
...
function derivedScoreEvents(match: any, sources: any[], homeScore: number, awayScore: number) {
  const savedEvents = match.events || [];
  if (savedEvents.length || homeScore + awayScore <= 0) return savedEvents;
  const minute = minuteFrom(match, sources);
  const leaderTeamId = homeScore > awayScore ? match.homeTeamId : awayScore > homeScore ? match.awayTeamId : null;
  return [{
    id: `derived-score-${match.id}-${homeScore}-${awayScore}`,
    minute,
    type: 'score_update',
    teamId: leaderTeamId,
    playerName: null,
    detail: `تحديث النتيجة المباشرة: ${match.homeTeam?.name || 'Home'} ${homeScore} - ${awayScore} ${match.awayTeam?.name || 'Away'} — اسم مسجل الهدف والتوقيت الرسمي غير متوفرين من timeline`,
  }];
}
...
export default async function MatchCenterPageLivePriority({ params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  const displayEvents = derivedScoreEvents(match, sources, Number(homeScore || 0), Number(awayScore || 0));
  const rows = [
