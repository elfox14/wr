...
type TabId = 'overview' | 'events' | 'stats' | 'lineups' | 'interactive' | 'analysis' | 'group' | 'articles';
type EventFilter = 'all' | 'goals' | 'cards' | 'subs' | 'home' | 'away' | 'critical';

const ar = new Intl.NumberFormat('ar-EG');
const statPriority = ['possession', 'xg', 'npxg', 'bigChances', 'shots', 'shotsOnTarget', 'shotsOffTarget', 'corners', 'yellowCards', 'redCards'];
const tabs: Array<{ id: TabId; label: string }> = [
  { id: 'overview', label: 'نظرة عامة' },
  { id: 'events', label: 'الأحداث' },
  { id: 'stats', label: 'كل الإحصائيات' },
  { id: 'lineups', label: 'اللاعبون' },
  { id: 'interactive', label: 'الملعب' },
...
function flagUrl(team: MatchPageData['homeTeam'], width = 160) {
...
function isTheStatsMetric(metric: MatchStatMetric) {
  return normalize(metric.source).includes('thestats') || normalize(metric.source).includes('the stats');
}
...
function availableMetrics(data: MatchPageData, onlyTheStats = false) {
  const rows = data.stats.filter((metric) => metric.available && (metric.home !== null || metric.away !== null));
  const theStats = rows.filter(isTheStatsMetric);
  if (onlyTheStats) return sortedMetrics(theStats.length ? theStats : rows);
  return sortedMetrics(rows);
}
...
function Flag({ team, small = false }: { team: MatchPageData['homeTeam']; small?: boolean }) {
...
function Tabs({ active, setActive, data, events }: { active: TabId; setActive: (id: TabId) => void; data: MatchPageData; events: MatchEventView[] }) {
  const counts: Partial<Record<TabId, number>> = { events: events.length, stats: availableMetrics(data, true).length, lineups: data.homePlayers.length + data.awayPlayers.length, articles: data.relatedArticles.length };
...
function OverviewPanel({ data, events }: { data: MatchPageData; events: MatchEventView[] }) {
  const theStatsMetrics = availableMetrics(data, true);
  const criticalEvents = events.filter((event) => eventMeta(event).critical).slice(-4);
  return <Panel title="نظرة عامة" hint="كل الإحصائيات النهائية المتوفرة من TheStats تظهر هنا بدون اختصار."><div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]"><div className="space-y-4"><div className="rounded-2xl border border-[#18E58F]/20 bg-[#18E58F]/[0.055] p-3"><div className="flex flex-wrap items-center justify-between gap-2"><h3 className="text-sm font-black text-[#18E58F]">الإحصائيات النهائية المتوفرة من TheStats</h3><span className="rounded-full border border-[#18E58F]/25 bg-black/20 px-3 py-1 text-[11px] font-black text-[#18E58F]">{ar.format(theStatsMetrics.length)} رقم متاح</span></div><p className="mt-1 text-xs font-bold leading-6 text-slate-400">يتم عرض كل المؤشرات المحفوظة والمتاحة، وليس أول 8 أو 12 رقمًا فقط.</p></div>{theStatsMetrics.length ? <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{theStatsMetrics.map((metric) => <StatCard key={metric.key} metric={metric} compact />)}</div> : <Empty title="لا توجد إحصائيات TheStats متاحة" body="لم يتم العثور على أرقام محفوظة من TheStats لهذه المباراة حتى الآن." />}</div><aside className="rounded-2xl border border-white/10 bg-black/25 p-3"><h3 className="mb-3 text-sm font-black text-[#F8C846]">لحظات مؤثرة</h3>{criticalEvents.length ? <div className="space-y-2">{criticalEvents.map((event) => <MiniEvent key={event.id} event={event} data={data} />)}</div> : <p className="text-xs font-bold leading-6 text-slate-400">لم يتم تسجيل لحظات حاسمة بعد.</p>}</aside></div></Panel>;
}
...
function EventsPanel({ data, events }: { data: MatchPageData; events: MatchEventView[] }) {
  ...
  return <Panel title="أحداث المباراة" hint="Timeline قابل للفلترة."><div className="space-y-3"><EventFilterBar active={filter} setActive={setFilter} data={data} events={events} />{rows.length ? <div className="space-y-2">{rows.map((event) => <EventRow key={event.id} event={event} data={data} />)}</div> : <Empty title="لا توجد أحداث مطابقة" body="جرّب فلترًا آخر أو تأكد من وصول أحداث المباراة." />}</div></Panel>;
}

function StatsPanel({ data }: { data: MatchPageData }) {
  const metrics = availableMetrics(data, true);
  return <Panel title="كل الإحصائيات" hint="كل الأرقام المتاحة من TheStats أو fallback محفوظ عند غياب مصدر TheStats.">{metrics.length ? <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{metrics.map((metric) => <StatCard key={metric.key} metric={metric} compact />)}</div> : <Empty title="لا توجد إحصائيات متاحة" body="لم تصل Snapshot إحصائية لهذه المباراة بعد." />}</Panel>;
}

function PlayersPanel({ data }: { data: MatchPageData }) {
  const stats = data.advanced.playerStats || [];
  const players = stats.length ? stats : [...data.homePlayers, ...data.awayPlayers].map((p) => ({ playerId: p.id, playerName: p.name, teamId: p.teamId, position: p.position })) as MatchPlayerStatItem[];
  return <Panel title="اللاعبون" hint="قائمة اللاعبين وتقييماتهم عند توفرها."><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{players.slice(0, 60).map((player, index) => <article key={`${player.playerId || player.playerName}-${index}`} className="rounded-2xl border border-white/10 bg-black/25 p-3"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><b className="block truncate text-sm text-white">{player.playerName}</b><p className="text-[11px] font-bold text-slate-500">{player.teamName || (player.teamId === data.homeTeam.id ? teamName(data.homeTeam) : player.teamId === data.awayTeam.id ? teamName(data.awayTeam) : '—')}</p></div>{player.rating !== null && player.rating !== undefined ? <span className="rounded-xl border border-[#18E58F]/20 bg-[#18E58F]/10 px-2 py-1 text-xs font-black text-[#18E58F]">{fmt(player.rating)}</span> : null}</div><div className="mt-3 grid grid-cols-3 gap-2 text-center text-[11px] font-bold text-slate-400"><span className="rounded-xl bg-white/[0.04] p-2"><b className="block text-white">{fmt(player.goals)}</b>أهداف</span><span className="rounded-xl bg-white/[0.04] p-2"><b className="block text-white">{fmt(player.assists)}</b>أسيست</span><span className="rounded-xl bg-white/[0.04] p-2"><b className="block text-white">{fmt(player.minutes)}</b>دقائق</span></div></article>)}</div></Panel>;
}

function InteractivePanel({ data, events }: { data: MatchPageData; events: MatchEventView[] }) {
  const points = events.slice(0, 80).map((event, index) => ({ event, x: event.x ?? (eventSide(event, data) === 'away' ? 25 : 75), y: event.y ?? (((Number(event.minute || index * 7) * 37 + index * 19) % 64) + 18) }));
  return <Panel title="الملعب التفاعلي" hint="عرض بصري تقديري للأحداث المتاحة.">{points.length ? <div className="relative min-h-[380px] overflow-hidden rounded-[1.6rem] border border-emerald-300/20 bg-[#0a3b25] p-3"><div className="absolute inset-4 rounded-[1.2rem] border-2 border-white/25" /><div className="absolute bottom-4 left-1/2 top-4 w-px bg-white/25" />{points.map(({ event, x, y }, index) => <span key={`${event.id}-${index}`} className="absolute z-10 flex h-9 w-9 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-white/40 bg-black/75 text-sm shadow-xl" style={{ left: `${x}%`, top: `${y}%` }} title={event.detail}>{eventMeta(event).icon}</span>)}</div> : <Empty title="لا توجد أحداث على الملعب" body="ستظهر بعد توفر أحداث المباراة." />}</Panel>;
}
