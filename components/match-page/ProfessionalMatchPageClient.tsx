...
import type { HeadToHeadMatchView, MatchEventView, MatchPageData, MatchPlayerLite, MatchPlayerStatItem, MatchShotMapItem, MatchStatMetric, OfficialLineupPlayer, OfficialLineupTeam, StandingRow, TeamRecentMatchView } from '@/lib/match-page/types';
...
function matchClockLabel(data: MatchPageData) { if (data.clock?.displayLabel && !data.status.isScheduled) return data.clock.displayLabel; if (data.status.isScheduled) return `موعد المباراة: ${fullDate(data.matchDate)}`; if (data.status.isFinished) return 'نهاية المباراة'; return data.status.label || 'زمن المباراة'; }
function resultLabel(value: TeamRecentMatchView['result']) { if (value === 'win') return 'فوز'; if (value === 'loss') return 'خسارة'; if (value === 'draw') return 'تعادل'; return 'لم تلعب'; }
function resultClass(value: TeamRecentMatchView['result']) { if (value === 'win') return 'border-[#18E58F]/30 bg-[#18E58F]/10 text-[#18E58F]'; if (value === 'loss') return 'border-rose-300/25 bg-rose-400/10 text-rose-200'; if (value === 'draw') return 'border-[#F8C846]/25 bg-[#F8C846]/10 text-[#F8C846]'; return 'border-white/10 bg-white/[0.06] text-slate-300'; }
...
function Hero({ data, onRefresh, onShare }: { data: MatchPageData; onRefresh: () => void; onShare: () => void }) {
  return <header className="relative overflow-hidden rounded-[1.5rem] border border-white/10 bg-[#04110D] p-3 text-center shadow-[0_24px_70px_rgba(0,0,0,.36)] sm:rounded-[2rem] sm:p-6"><div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_12%,rgba(24,229,143,.20),transparent_32%),radial-gradient(circle_at_82%_8%,rgba(248,200,70,.14),transparent_30%),linear-gradient(180deg,rgba(255,255,255,.06),transparent_42%)]" /><div className="relative"><div className="grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2 sm:gap-5" dir="rtl"><TeamSide team={data.homeTeam} /><div className="space-y-2"><div className="inline-flex items-center justify-center gap-2 rounded-2xl border border-white/10 bg-black/45 px-3 py-2 shadow-inner sm:gap-5 sm:px-7 sm:py-3"><span className="text-3xl font-black text-[#F8C846] tabular-nums sm:text-7xl">{fmt(data.score.home)}</span><span className="text-2xl font-black text-white/70 sm:text-6xl">-</span><span className="text-3xl font-black text-white tabular-nums sm:text-7xl">{fmt(data.score.away)}</span></div><p className={`mx-auto inline-flex rounded-full border px-3 py-1.5 text-xs font-black sm:px-4 sm:py-2 ${statusClasses[data.status.kind]}`}>{matchClockLabel(data)}</p></div><TeamSide team={data.awayTeam} /></div><div className="mt-5 grid grid-cols-2 gap-2 text-right lg:grid-cols-6"><Info icon={<MapPin size={15} />} label="الملعب" value={data.venue || 'غير محفوظ'} /><Info icon={<Shield size={15} />} label="المدينة" value={data.city || '—'} /><Info icon={<Flag size={15} />} label="الحكم" value={data.referee || '—'} /><Info icon={<Users size={15} />} label={`مدرب ${data.homeTeam.name}`} value={data.homeTeam.coach || '—'} /><Info icon={<Users size={15} />} label={`مدرب ${data.awayTeam.name}`} value={data.awayTeam.coach || '—'} /><Info icon={<Trophy size={15} />} label="المجموعة" value={data.groupLabel || data.stageLabel} /></div><div className="mt-4 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:items-center sm:justify-center"><button onClick={onRefresh} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#18E58F] px-3 py-2 text-xs font-black text-black sm:text-sm"><RefreshCw size={16} /> تحديث</button><button onClick={onShare} className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/10 px-3 py-2 text-xs font-black text-white sm:text-sm"><Share2 size={16} /> مشاركة</button>{data.digest?.href ? <Link href={data.digest.href} className="col-span-2 inline-flex items-center justify-center gap-2 rounded-xl border border-[#F8C846]/30 bg-[#F8C846]/12 px-3 py-2 text-xs font-black text-[#F8C846] sm:col-span-1 sm:text-sm"><FileText size={16} /> تقرير المباراة</Link> : null}</div></div></header>;
}

function ScoreText({ score }: { score: MatchPageData['score'] }) {
  return <span className="tabular-nums">{fmt(score.home)} - {fmt(score.away)}</span>;
}

function RecentMatchLine({ item }: { item: TeamRecentMatchView }) {
  return <article className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.045] p-2"><FlagImg team={{ id: item.id, name: item.opponentName, image: item.opponentImage }} small /><div className="min-w-0"><p className="truncate text-sm font-black text-white">{item.opponentName}</p><p className="mt-0.5 text-[10px] font-bold text-slate-500">{fullDate(item.date)} · {item.side === 'home' ? 'على أرضه' : 'خارج أرضه'}</p></div><div className="text-left"><b className="block text-xs font-black text-white"><ScoreText score={item.score} /></b><span className={`mt-1 inline-flex rounded-full border px-2 py-0.5 text-[10px] font-black ${resultClass(item.result)}`}>{resultLabel(item.result)}</span></div></article>;
}

function FormPanel({ title, rows }: { title: string; rows: TeamRecentMatchView[] }) {
  return <div className="rounded-[1.35rem] border border-white/10 bg-black/20 p-3"><h3 className="mb-3 text-sm font-black text-[#F8C846]">{title}</h3>{rows.length ? <div className="space-y-2">{rows.map((item) => <RecentMatchLine key={item.id} item={item} />)}</div> : <Empty title="لا توجد نتائج محفوظة" body="سيظهر آخر سجل مباريات عند توفره في قاعدة البيانات." />}</div>;
}

function HeadToHeadLine({ item }: { item: HeadToHeadMatchView }) {
  return <article className="rounded-2xl border border-white/10 bg-white/[0.045] p-2"><div className="grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2 text-center"><span className="truncate text-xs font-black text-white">{item.homeName}</span><b className="rounded-xl bg-black/35 px-2 py-1 text-xs text-[#F8C846]"><ScoreText score={item.score} /></b><span className="truncate text-xs font-black text-white">{item.awayName}</span></div><p className="mt-1 text-center text-[10px] font-bold text-slate-500">{fullDate(item.date)}</p></article>;
}

function PreMatchContext({ data }: { data: MatchPageData }) {
  return <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(280px,.7fr)]"><div className="grid gap-4 md:grid-cols-2"><FormPanel title={`آخر 5 مباريات - ${data.homeTeam.name}`} rows={data.homeForm} /><FormPanel title={`آخر 5 مباريات - ${data.awayTeam.name}`} rows={data.awayForm} /></div><div className="rounded-[1.35rem] border border-white/10 bg-black/20 p-3"><div className="mb-3 flex items-center justify-between gap-2"><h3 className="text-sm font-black text-[#18E58F]">تاريخ المواجهات</h3><span className="rounded-full border border-white/10 bg-white/[0.05] px-2 py-1 text-[10px] font-black text-slate-300">{data.clock.lastConfirmedAt ? `آخر تأكيد ${fullDate(data.clock.lastConfirmedAt)}` : 'DB فقط'}</span></div>{data.headToHead.length ? <div className="space-y-2">{data.headToHead.map((item) => <HeadToHeadLine key={item.id} item={item} />)}</div> : <Empty title="لا توجد مواجهات محفوظة" body="لا يتم طلب بيانات تاريخية من APIs أثناء فتح الصفحة." />}{data.clock.note ? <p className="mt-3 rounded-xl border border-white/10 bg-white/[0.04] p-2 text-xs font-bold leading-6 text-slate-400">{data.clock.note}</p> : null}</div></section>;
}
...
export default function ProfessionalMatchPageClient({ data }: { data: MatchPageData }) {
  ...
  return <main className="min-h-screen bg-[#04110D] px-2 pb-20 pt-3 text-white sm:px-4 sm:pt-4" dir="rtl"><MatchAutoRefresh intervalMs={refreshMs} /><div className="mx-auto max-w-7xl space-y-4 sm:space-y-5"><Hero data={data} onRefresh={refresh} onShare={share} /><PreMatchContext data={data} /><StickyTabs active={activeTab} onSelect={selectTab} /><EventsPanel data={data} /><StatsPanel data={data} /><LineupsPanel data={data} /><StandingsPanel data={data} /><AnalysisPanel data={data} /></div></main>;
}
