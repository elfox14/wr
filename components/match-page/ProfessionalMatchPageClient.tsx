...
function pct(home: number | null, away: number | null) {
...
function matchClockLabel(data: MatchPageData) {
  if (data.status.isScheduled) return `موعد المباراة: ${fullDate(data.matchDate)}`;
  if (data.status.isFinished) return 'نهاية المباراة';
  return data.status.label || 'زمن المباراة';
}
function findLocalPlayer(player: PitchPlayer, localPlayers: MatchPlayerLite[]) {
  const id = 'id' in player && player.id ? String(player.id) : null;
...
function teamSideForId(data: MatchPageData, teamId?: string | null): PitchSide | null {
  if (teamId === data.homeTeam.id) return 'home';
  if (teamId === data.awayTeam.id) return 'away';
  return null;
}
...
function playerStatFor(name: string | null | undefined, stats: MatchPlayerStatItem[]) {
  const key = normalizeName(name);
  if (!key) return null;
  return stats.find((item) => {
    const itemName = normalizeName(item.playerName);
    return itemName && (itemName === key || itemName.includes(key) || key.includes(itemName));
  }) || null;
}
function isChangeEvent(event: MatchEventView) {
  const text = normalizeName(`${event.type} ${event.detail}`);
  return text.includes('substitution') || text.includes('sub') || text.includes('تبديل') || text.includes('دخول') || text.includes('خروج');
}
function eventTeamPlayers(event: MatchEventView, data: MatchPageData) {
  if (event.teamId === data.homeTeam.id) return data.homePlayers;
  if (event.teamId === data.awayTeam.id) return data.awayPlayers;
  return [...data.homePlayers, ...data.awayPlayers];
}
function playerFromChange(event: MatchEventView, data: MatchPageData): PitchPlayer | null {
  const name = event.playerName || event.detail?.split(' - ').pop() || null;
  if (!name) return null;
  return playerWithRealImage({ name }, eventTeamPlayers(event, data));
}
...
function Info({ icon, label, value }: { icon: ReactNode; label: string; value: string }) {
  return <div className="min-w-0 rounded-2xl border border-white/10 bg-black/25 p-2.5 text-center sm:p-3 sm:text-right"><div className="mb-1 flex items-center justify-center gap-1.5 text-[#18E58F] sm:justify-start">{icon}<span className="text-[10px] font-black sm:text-xs">{label}</span></div><p className="line-clamp-2 min-h-[2.35rem] text-[10px] font-black leading-5 text-white sm:min-h-0 sm:text-sm sm:font-bold sm:leading-6">{value || '—'}</p></div>;
}
...
function slotsFor(players: PitchPlayer[], formation: string | null | undefined, side: PitchSide): PitchSlot[] {
  ...
  lines.forEach((lineCount, lineIndex) => {
    ...
    for (let i = 0; i < actual; i += 1) {
      ...
      const x = side === 'home' ? 86 - progress * 30 : 14 + progress * 30;
      const y = actual === 1 ? 50 : 14 + (72 * (i + 0.5)) / actual;
...
function ChangePlayerToken({ event, data, side, index, total }: { event: MatchEventView; data: MatchPageData; side: PitchSide; index: number; total: number }) {
  ...
  return <div className="absolute z-30 flex w-16 -translate-x-1/2 -translate-y-1/2 flex-col items-center text-center" style={{ left: `${x}%`, top: `${y}%` }} title={event.detail}><div className={`relative flex h-9 w-9 items-center justify-center overflow-hidden rounded-full border-2 shadow-xl ${side === 'home' ? 'border-[#F8C846] bg-black/50' : 'border-[#18E58F] bg-black/50'}`}>{image ? <img src={image} alt={player?.name || event.playerName || 'تبديل'} className="h-full w-full object-cover" loading="lazy" /> : <span className="text-[10px] font-black text-white">{initials(player?.name || event.playerName || 'تبديل')}</span>}</div><span className="mt-1 line-clamp-2 rounded-md bg-black/65 px-1 py-0.5 text-[8px] font-black leading-3 text-white backdrop-blur">{player?.name || event.playerName || 'تبديل'}</span><span className="mt-0.5 rounded-full bg-[#F8C846] px-1.5 py-0.5 text-[8px] font-black text-black">{event.minuteLabel}</span></div>;
}
function OfficialPitch({ data }: { data: MatchPageData }) {
  ...
  return <div className="relative overflow-hidden rounded-[1.6rem] border border-[#18E58F]/25 bg-[#0b3b28] p-3"><div className="mb-3 flex items-center justify-between gap-2 rounded-2xl bg-black/25 px-3 py-2 text-xs font-black text-white"><span>{data.awayTeam.name} · {awayFormation}</span><span className="text-[#F8C846]">التشكيلة الرسمية على الملعب</span><span>{data.homeTeam.name} · {homeFormation}</span></div><div className="relative h-[620px] overflow-hidden rounded-[1.35rem] border-2 border-white/35 bg-[linear-gradient(90deg,rgba(255,255,255,.05)_0_50%,rgba(255,255,255,.08)_50%_100%)] sm:h-[680px] lg:h-[560px]"><div className="absolute inset-y-0 left-1/2 w-px -translate-x-1/2 bg-white/45" /><div className="absolute left-1/2 top-1/2 h-28 w-28 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-white/35" /><div className="absolute left-0 top-1/2 h-40 w-16 -translate-y-1/2 rounded-r-3xl border-y-2 border-r-2 border-white/30" /><div className="absolute right-0 top-1/2 h-40 w-16 -translate-y-1/2 rounded-l-3xl border-y-2 border-l-2 border-white/30" />{awayChanges.length ? <div className="absolute left-2 top-2 z-20 rounded-full border border-[#18E58F]/30 bg-black/45 px-2 py-1 text-[9px] font-black text-[#18E58F]">تغييرات {data.awayTeam.name}</div> : null}{homeChanges.length ? <div className="absolute right-2 top-2 z-20 rounded-full border border-[#F8C846]/30 bg-black/45 px-2 py-1 text-[9px] font-black text-[#F8C846]">تغييرات {data.homeTeam.name}</div> : null}{awayChanges.map((event, index) => <ChangePlayerToken key={`away-change-${event.id}`} event={event} data={data} side="away" index={index} total={awayChanges.length} />)}{homeChanges.map((event, index) => <ChangePlayerToken key={`home-change-${event.id}`} event={event} data={data} side="home" index={index} total={homeChanges.length} />)}{slots.length ? slots.map((slot, index) => <PitchPlayerToken key={`${slot.side}-${slot.player.name}-${index}`} slot={slot} />) : <div className="absolute inset-0 grid place-items-center p-6"><Empty title="التشكيل لم يصل بعد" body="سيظهر تلقائيًا على الملعب عند وصول التشكيل الرسمي من مزودي البيانات." /></div>}</div></div>;
}
function PlayerStatsTable({ data }: { data: MatchPageData }) {
  const stats = data.advanced.playerStats || [];
  const officialNames = [...(data.officialLineup?.home?.startingXi || []), ...(data.officialLineup?.home?.substitutes || []), ...(data.officialLineup?.away?.startingXi || []), ...(data.officialLineup?.away?.substitutes || [])];
  const rows = officialNames.map((player) => ({ player, stat: playerStatFor(player.name, stats) })).filter((row) => row.stat);
  if (!rows.length) return null;
  return <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-3"><div className="mb-3 flex items-center justify-between"><h3 className="font-black text-white">إحصائيات لاعبي التشكيل والبدلاء</h3><span className="text-xs font-black text-[#F8C846]">{ar.format(rows.length)} لاعب</span></div><div className="max-h-[430px] overflow-x-auto overflow-y-auto"><table className="w-full min-w-[860px] border-separate border-spacing-y-2 text-right text-xs"><thead className="text-slate-500"><tr><th className="px-2">اللاعب</th><th>تقييم</th><th>دقائق</th><th>أهداف</th><th>أسيست</th><th>تسديدات</th><th>تمريرات</th><th>Key passes</th><th>تدخلات</th><th>تصديات</th></tr></thead><tbody>{rows.map(({ player, stat }, index) => <tr key={`${player.name}-${index}`} className="bg-white/[0.045] text-white"><td className="rounded-r-xl px-2 py-3 font-black">{player.name}</td><td>{fmt(stat?.rating)}</td><td>{fmt(stat?.minutes)}</td><td>{fmt(stat?.goals)}</td><td>{fmt(stat?.assists)}</td><td>{fmt(stat?.shots)}</td><td>{fmt(stat?.passes)}</td><td>{fmt(stat?.keyPasses)}</td><td>{fmt(stat?.tackles)}</td><td className="rounded-l-xl">{fmt(stat?.saves)}</td></tr>)}</tbody></table></div></div>;
}
function LineupsPanel({ data }: { data: MatchPageData }) {
  const official = data.officialLineup;
  return <Section id="lineups" title="التشكيل الرسمي" icon={<Users size={22} />} hint={official ? `خطة ${data.homeTeam.name}: ${official.home?.formation || 'غير متوفر'} · خطة ${data.awayTeam.name}: ${official.away?.formation || 'غير متوفر'}` : 'سيتم جلب التشكيل الرسمي تلقائيًا عند توفره'}><OfficialPitch data={data} /><PlayerStatsTable data={data} /></Section>;
}

function StatMini({ label, value, accent = false }: { label: string; value: ReactNode; accent?: boolean }) {
  return <div className="rounded-xl border border-white/10 bg-black/25 px-2 py-2 text-center"><span className="block text-[10px] font-black text-slate-500">{label}</span><b className={`mt-1 block text-sm font-black tabular-nums ${accent ? 'text-[#18E58F]' : 'text-white'}`}>{value}</b></div>;
}
function StandingCard({ row, compact = false }: { row: StandingRow; compact?: boolean }) {
  const gdPositive = row.goalDifference > 0;
