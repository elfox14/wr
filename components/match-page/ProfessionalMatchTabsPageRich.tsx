...
function History({ d }: { d: MatchPageData }) {
  ...
  return (
    <div className="space-y-6">
      <Box title="تاريخ لقاءات المنتخبين" hint="النتائج من قاعدة المباريات الموثقة؛ ويمكن مراجعة الأرشيف الدولي الرسمي لدى FIFA.">
        <div className="mb-4 flex justify-end"><a href="https://inside.fifa.com/data-centre/matches" target="_blank" rel="noreferrer" className="rounded-full border border-[#18E58F]/25 bg-[#18E58F]/10 px-3 py-1.5 text-[10px] font-black text-[#18E58F] hover:bg-[#18E58F]/20">فتح أرشيف مباريات FIFA ↗</a></div>
      ...
      <Box title="تاريخ المنتخبين في كأس العالم" hint="الأرقام والسنوات المعلّمة بـ FIFA مأخوذة من الملفات الرسمية للمنتخبات.">
        <div className="grid gap-4 md:grid-cols-2">
          <article className="rounded-3xl border border-[#0FF0FC]/20 bg-[#0FF0FC]/5 p-5"><div className="mb-4 flex items-center gap-3"><TeamFlag team={d.homeTeam} /><h3 className="text-lg font-black text-[#0FF0FC]">{tn(d.homeTeam)}</h3></div><p className="text-sm font-bold leading-7 text-slate-300">{history.homeWorldCupHistory}</p>{history.homeWorldCupHistoryUrl ? <a href={history.homeWorldCupHistoryUrl} target="_blank" rel="noreferrer" className="mt-4 inline-flex rounded-full border border-[#0FF0FC]/25 bg-black/25 px-3 py-1 text-[10px] font-black text-[#0FF0FC]">المصدر الرسمي: FIFA ↗</a> : null}</article>
          <article className="rounded-3xl border border-[#F8C846]/20 bg-[#F8C846]/5 p-5"><div className="mb-4 flex items-center gap-3"><TeamFlag team={d.awayTeam} /><h3 className="text-lg font-black text-[#F8C846]">{tn(d.awayTeam)}</h3></div><p className="text-sm font-bold leading-7 text-slate-300">{history.awayWorldCupHistory}</p>{history.awayWorldCupHistoryUrl ? <a href={history.awayWorldCupHistoryUrl} target="_blank" rel="noreferrer" className="mt-4 inline-flex rounded-full border border-[#F8C846]/25 bg-black/25 px-3 py-1 text-[10px] font-black text-[#F8C846]">المصدر الرسمي: FIFA ↗</a> : null}</article>
        </div>
      </Box>
    </div>
  );
