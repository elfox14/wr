...
function isTheStatsMetric(metric: MatchStatMetric) {
  const source = normalize(metric.source);
  return source.includes('thestats') || source.includes('the stats');
}
...
function availableTheStatsMetrics(data: MatchPageData) {
  return sortedMetrics(data.stats.filter((metric) => metric.available && isTheStatsMetric(metric) && (metric.home !== null || metric.away !== null)));
}

function availableAnyMetrics(data: MatchPageData) {
...
function QuickStats({ data }: { data: MatchPageData }) {
  const metrics = (availableTheStatsMetrics(data).length ? availableTheStatsMetrics(data) : availableAnyMetrics(data)).slice(0, 5);
...
function Tabs({ active, setActive, data, events }: { active: TabId; setActive: (id: TabId) => void; data: MatchPageData; events: MatchEventView[] }) {
  const counts: Partial<Record<TabId, number>> = { events: events.length, stats: availableTheStatsMetrics(data).length, lineups: participatedPlayers(data).length, articles: data.relatedArticles.length };
...
function OverviewPanel({ data, events }: { data: MatchPageData; events: MatchEventView[] }) {
  const metrics = availableTheStatsMetrics(data);
  const criticalEvents = events.filter((event) => eventMeta(event).critical).slice(-4);
  return <Panel title="نظرة عامة" hint="كل الإحصائيات النهائية المتوفرة من TheStats تظهر هنا بدون اختصار."><div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]"><div className="space-y-4"><div className="rounded-2xl border border-[#18E58F]/20 bg-[#18E58F]/[0.055] p-3"><div className="flex flex-wrap items-center justify-between gap-2"><h3 className="text-sm font-black text-[#18E58F]">الإحصائيات النهائية المتوفرة من TheStats</h3><span className="rounded-full border border-[#18E58F]/25 bg-black/20 px-3 py-1 text-[11px] font-black text-[#18E58F]">{ar.format(metrics.length)} رقم متاح</span></div><p className="mt-1 text-xs font-bold leading-6 text-slate-400">لا يتم قص الإحصائيات هنا. أي رقم نهائي محفوظ من TheStats سيظهر في النظرة العامة.</p></div>{metrics.length ? <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{metrics.map((metric) => <StatCard key={metric.key} metric={metric} compact />)}</div> : <Empty title="لا توجد إحصائيات TheStats متاحة" body="لم يتم العثور على أرقام نهائية محفوظة من TheStats لهذه المباراة حتى الآن." />}</div><aside className="rounded-2xl border border-white/10 bg-black/25 p-3"><h3 className="mb-3 text-sm font-black text-[#F8C846]">لحظات مؤثرة</h3>{criticalEvents.length ? <div className="space-y-2">{criticalEvents.map((event) => <MiniEvent key={event.id} event={event} data={data} />)}</div> : <p className="text-xs font-bold leading-6 text-slate-400">لم يتم تسجيل لحظات حاسمة بعد.</p>}</aside></div></Panel>;
}
...
export default function ProfessionalMatchTabsPageFullStatsV2({ data }: { data: MatchPageData }) {
