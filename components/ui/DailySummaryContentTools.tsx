...
import { Archive, Copy, FileText, Image as ImageIcon, Loader2, Sparkles, Video } from 'lucide-react';
...
export default function DailySummaryContentTools({ script, headline, summaryLine, digestDate }: Props) {
  ...
  return <section className="rounded-[2rem] border border-[#FFD700]/15 bg-[#FFD700]/[0.035] p-5 md:p-6">
    <div className="mb-4 flex flex-col gap-2 md:flex-row md:items-center md:justify-between"><div><h2 className="flex items-center gap-2 text-2xl font-black"><Sparkles className="text-[#FFD700]" /> باقة محتوى ملخص اليوم</h2><p className="mt-1 text-xs font-bold text-gray-500">انسخ السكربت أو احفظ الملخص في الأرشيف.</p></div><div className="flex flex-wrap items-center gap-2">{copied && <span className="rounded-full border border-emerald-400/20 bg-emerald-400/10 px-3 py-2 text-xs font-black text-emerald-300">تم النسخ</span>}<button type="button" onClick={saveDigest} disabled={saving} className="inline-flex items-center gap-2 rounded-2xl border border-[#FFD700]/25 bg-[#FFD700]/10 px-4 py-3 text-xs font-black text-[#FFD700] transition hover:bg-[#FFD700] hover:text-black disabled:opacity-60">{saving ? <Loader2 className="animate-spin" size={15} /> : <Archive size={15} />} حفظ في الأرشيف</button></div></div>
