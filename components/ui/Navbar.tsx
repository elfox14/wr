...
import { CalendarDays, ChevronDown, FileText, LogOut, Menu, Newspaper, Radio, Shield, ShieldAlert, Trophy, User as UserIcon, Users, X, Brain, Home } from 'lucide-react';

const adminEmails = new Set(['worldcup@mcprim.com', 'elfox14usa@gmail.com']);
const headerLogo = '/brand/worldcup-exchange-header-logo.svg?v=wc2026-sports-only';
...
export function Navbar() {
  ...
  const navLinks = [
    { name: 'الرئيسية', href: '/', icon: <Home size={16} />, exact: true },
    { name: 'الأخبار', href: '/news', icon: <Newspaper size={16} /> },
    { name: 'المجموعات', href: '/groups', icon: <Trophy size={16} /> },
    { name: 'المنتخبات', href: '/teams', icon: <Shield size={16} /> },
    { name: 'اللاعبون', href: '/players', icon: <Users size={16} /> },
    { name: 'المباريات', href: '/matches', icon: <CalendarDays size={16} /> },
    { name: 'التحليل', href: '/team-intelligence', icon: <Brain size={16} /> },
                ...
                <div className="hidden items-center gap-3 lg:flex">
                  <div className="relative" ref={userMenuRef}>
                    <button onClick={() => setIsUserMenuOpen(!isUserMenuOpen)} className="flex items-center gap-2 rounded-full border border-white/10 bg-white/5 p-1.5 pr-3 transition-all hover:bg-white/10">
                      <span className="hidden max-w-[120px] truncate text-xs font-bold text-white xl:block">{session.user?.name?.split(' ')[0] || session.user?.email?.split('@')[0]}</span>
