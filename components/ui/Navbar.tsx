      ...
      <nav className="fixed top-0 z-50 w-full border-b border-white/10 bg-black/80 shadow-[0_8px_30px_rgba(0,0,0,0.35)] backdrop-blur-xl">
          ...
          <div className="flex h-16 items-center justify-between lg:h-20">
              ...
              <div className="relative" ref={exchangeMenuRef}>
                ...
                <div className={`absolute left-0 top-full mt-2 w-56 overflow-hidden rounded-2xl border border-white/10 bg-[#121212] shadow-2xl transition-all duration-200 ${isExchangeMenuOpen ? 'visible scale-100 opacity-100' : 'invisible scale-95 opacity-0'}`}>
                  <div className="border-b border-white/5 px-4 py-3">
                    <p className="text-xs font-black text-[#FFD700]">كل ما يخص التداول</p>
                  </div>
                  <div className="space-y-1 p-2">
                    {exchangeLinks.map((link) => (
                      <DropdownLink key={link.href} href={link.href} icon={link.icon} label={link.name} gold={link.href !== '/portfolio'} />
                    ))}
                  </div>
                </div>
              </div>
            ...
            <div className="flex items-center gap-3">
              {status === 'loading' ? (
                  ...
                  <div className="relative" ref={userMenuRef}>
                    ...
                    <div className={`absolute left-0 top-full mt-2 w-64 origin-top-left overflow-hidden rounded-2xl border border-white/10 bg-[#121212] shadow-2xl backdrop-blur-xl transition-all duration-200 ${isUserMenuOpen ? 'visible scale-100 opacity-100' : 'invisible scale-95 opacity-0'}`}>
                      ...
                      <div className="space-y-1 p-2">
                        ...
                        {isAdminUser && (
                          <>
                            <DropdownLink href="/admin" icon={<ShieldAlert size={16} />} label="الإدارة" danger />
                            <DropdownLink href="/admin/content-studio" icon={<FileText size={16} />} label="استوديو المحتوى" gold />
                            <DropdownLink href="/admin/news" icon={<Newspaper size={16} />} label="إدارة الأخبار" cyan />
                            <DropdownLink href="/admin/match-events" icon={<Trophy size={16} />} label="إدارة أحداث المباراة" gold />
                          </>
                        )}
                      </div>
                      <div className="border-t border-white/5 p-2">
                        <button onClick={() => signOut({ callbackUrl: '/' })} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-gray-400 transition-colors hover:bg-red-500/10 hover:text-red-500"><LogOut size={16} /> تسجيل الخروج</button>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="hidden items-center gap-3 lg:flex">
                  <Link href="/login" className="px-4 py-2 text-sm font-bold text-gray-300 transition-colors hover:text-white">تسجيل الدخول</Link>
                  <Link href="/register" className="rounded-full bg-[#0FF0FC] px-6 py-2 text-sm font-black text-black shadow-[0_0_15px_rgba(15,240,252,0.3)] transition-all hover:bg-[#0FF0FC]/80">إنشاء حساب</Link>
                </div>
              )}
        ...
        {isMobileMenuOpen && (
          <div className="border-t border-white/10 bg-black/95 backdrop-blur-xl lg:hidden">
            <div className="border-b border-white/10 px-4 py-3 text-xs font-bold leading-5 text-gray-300">{slogan}</div>
            <div className="space-y-2 px-4 py-4">
              ...
              <div className="rounded-2xl border border-[#FFD700]/20 bg-[#FFD700]/[0.04] p-2">
                <div className="px-2 pb-2 text-xs font-black text-[#FFD700]">البورصة — كل ما يخص التداول</div>
                {exchangeLinks.map((link) => <Link key={link.href} href={link.href} onClick={closeMobileMenu} className="flex items-center gap-3 rounded-xl px-4 py-3 font-bold text-gray-300 transition-colors hover:bg-white/5 hover:text-white">{link.icon}{link.name}</Link>)}
              </div>
              {session && (
                <>
                  <Link href="/daily-summary" onClick={closeMobileMenu} className="flex items-center gap-3 rounded-xl border border-[#0FF0FC]/20 bg-[#0FF0FC]/10 px-4 py-3 font-bold text-[#0FF0FC]"><CalendarDays size={16} /> ملخص اليوم</Link>
                  <Link href="/match-digests" onClick={closeMobileMenu} className="flex items-center gap-3 rounded-xl border border-[#0FF0FC]/20 bg-[#0FF0FC]/10 px-4 py-3 font-bold text-[#0FF0FC]"><FileText size={16} /> ملخصات المباريات</Link>
                  {isAdminUser && <Link href="/admin" onClick={closeMobileMenu} className="flex items-center gap-3 rounded-xl border border-red-500/20 bg-red-500/10 px-4 py-3 font-bold text-red-400"><ShieldAlert size={16} /> الإدارة</Link>}
                  <button onClick={() => signOut({ callbackUrl: '/' })} className="flex w-full items-center gap-3 rounded-xl px-4 py-3 font-bold text-gray-400 hover:bg-red-500/10 hover:text-red-500"><LogOut size={16} /> تسجيل الخروج</button>
                </>
              )}
            </div>
          </div>
...
function DropdownLink({ href, icon, label, cyan, gold, danger }: { href: string; icon: ReactNode; label: string; cyan?: boolean; gold?: boolean; danger?: boolean }) {
