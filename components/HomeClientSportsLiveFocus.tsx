...
function liveStateLabel(match: HomeMatch) {
  ...
  const status = normalizeStatus(match);
  if (status === '1H') return minute ? `الشوط الأول — د${formatCount(minute)}` : 'الشوط الأول';
  if (status === '2H') return minute ? `الشوط الثاني — د${formatCount(minute)}` : 'الشوط الثاني';
  if (status === 'ET') return minute ? `وقت إضافي — د${formatCount(minute)}` : 'وقت إضافي';
  if (status === 'P' || status === 'PEN') return 'ركلات الترجيح';
  return minute ? `جارية الآن — د${formatCount(minute)}` : 'جارية الآن';
}
    ...
    <section className="overflow-hidden rounded-[1.6rem] border border-[#FFD700]/15 bg-[radial-gradient(circle_at_top,rgba(255,215,0,0.10),transparent_35%),linear-gradient(180deg,rgba(255,255,255,0.055),rgba(0,0,0,0.22))] p-3 shadow-[0_18px_50px_rgba(0,0,0,0.28)] backdrop-blur sm:p-4">
      {primary ? (
        ...
        <div className="rounded-3xl border border-white/10 bg-black/25 p-5 text-center">
          <p className="text-sm font-bold text-gray-300">لا توجد مباريات قريبة للعرض الآن.</p>
          <Link href="/matches" className="mt-3 inline-flex rounded-full bg-[#FFD700] px-4 py-2 text-xs font-black text-black">عرض جدول المباريات</Link>
        </div>
      )}
    </section>
  );
