...
function teamLabel(team?: Team | null) { return team?.name || team?.code || 'منتخب غير محدد'; }
...
function getBroadcastHref(match: HomeMatch) { return match.id ? `/match-center/${encodeURIComponent(String(match.id))}` : getMatchHref(match); }
...
function minutesSinceKickoff(match: HomeMatch, now: Date) { const date = match.matchDate ? new Date(match.matchDate) : null; if (!date || !Number.isFinite(date.getTime())) return null; return Math.floor((now.getTime() - date.getTime()) / 60_000) + 1; }
function hasKickoffPassed(match: HomeMatch, now: Date) { const minutes = minutesSinceKickoff(match, now); return minutes !== null && minutes >= 1; }
function isFinished(match?: HomeMatch | null) { return FINISHED_STATUSES.includes(normalizeStatus(match)); }
...
function isScheduled(match?: HomeMatch | null) { return !isFinished(match) && SCHEDULED_STATUSES.includes(normalizeStatus(match)); }
...
function formatKickoffTime(value?: string | Date | null) { if (!value) return '—'; const date = new Date(value); if (!Number.isFinite(date.getTime())) return '—'; return new Intl.DateTimeFormat('ar-EG', { hour: '2-digit', minute: '2-digit' }).format(date); }
function countdownParts(match: HomeMatch, now: Date) { const date = match.matchDate ? new Date(match.matchDate) : null; const diffMs = date && Number.isFinite(date.getTime()) ? date.getTime() - now.getTime() : 0; const totalSeconds = Math.max(0, Math.floor(diffMs / 1000)); return { active: diffMs > 0, days: Math.floor(totalSeconds / 86_400), hours: Math.floor((totalSeconds % 86_400) / 3_600), minutes: Math.floor((totalSeconds % 3_600) / 60), seconds: totalSeconds % 60 }; }
...
function TeamBadge({ team, align }: { team?: Team | null; align: 'right' | 'left' }) {
  ...
  return <Link href={getTeamHref(team)} className={`group/team flex min-w-0 items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.04] px-2.5 py-2 transition hover:border-[#FFD700]/25 hover:bg-white/[0.07] sm:border-transparent sm:bg-transparent sm:p-1.5 ${align === 'left' ? 'flex-row-reverse text-left' : 'text-right'}`}><span className="h-8 w-8 shrink-0 rounded-xl border border-white/10 bg-cover bg-center bg-no-repeat shadow-[0_8px_18px_rgba(0,0,0,0.22)] sm:h-9 sm:w-9" style={src ? { backgroundImage: `url(${src})` } : undefined}>{!src ? <span className="flex h-full w-full items-center justify-center text-[10px] font-black text-[#FFD700]">{teamCode(team)}</span> : null}</span><span className="min-w-0"><span className="block truncate text-[11px] font-black leading-4 text-white sm:text-xs">{teamLabel(team)}</span><span className="mt-0.5 block text-[9px] font-bold text-gray-500">{teamCode(team)}</span></span></Link>;
}
...
function MatchRow({ match, now, variant = 'normal' }: { match: HomeMatch; now: Date; variant?: 'live' | 'primary' | 'normal' }) {
  ...
  return <article className={`relative overflow-hidden rounded-[1.35rem] border p-3 transition sm:rounded-3xl sm:p-3.5 ${shell}`}>{isPrimary ? <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#FFD700]/60 to-transparent" /> : null}<div className="mb-3 grid grid-cols-2 gap-1.5 sm:flex sm:flex-wrap sm:items-center sm:justify-between"><span className="rounded-full border border-[#FFD700]/20 bg-[#FFD700]/10 px-2.5 py-1 text-center text-[10px] font-black text-[#FFD700]">{groupNumberLabel(match)}</span><span className="rounded-full border border-white/10 bg-white/[0.06] px-2.5 py-1 text-center text-[10px] font-black text-gray-200">{formatMatchDate(match.matchDate)}</span><span className="rounded-full border border-[#0FF0FC]/20 bg-[#0FF0FC]/10 px-2.5 py-1 text-center text-[11px] font-black text-[#0FF0FC]">{formatKickoffTime(match.matchDate)}</span><span className="flex justify-center sm:block"><MatchStatePill match={match} now={now} /></span></div><div className="grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-1.5 sm:gap-2.5"><TeamBadge team={match.homeTeam} align="right" /><MatchScore match={match} /><TeamBadge team={match.awayTeam} align="left" /></div><div className="mt-3"><Link href={getBroadcastHref(match)} className="mobile-tap inline-flex w-full items-center justify-center rounded-xl bg-[#0FF0FC] px-3 py-2.5 text-center text-[11px] font-black text-black transition hover:bg-[#4AFAFF]">البث التفاعلي</Link></div></article>;
}
function MatchCenter({ fallbackMatches = [], nextMatch = null }: { fallbackMatches?: HomeMatch[]; nextMatch?: HomeMatch | null }) {
  ...
  useEffect(() => { let cancelled = false; async function loadMatches() { if (typeof document !== 'undefined' && document.visibilityState === 'hidden') return; try { const response = await fetch('/api/matches/live-card', { cache: 'no-store' }); if (!response.ok) return; const data = await response.json(); const list = Array.isArray(data) ? data : Array.isArray(data?.matches) ? data.matches : []; if (!cancelled) setMatches(list); } catch {} } loadMatches(); const timer = window.setInterval(loadMatches, MATCH_REFRESH_MS); return () => { cancelled = true; window.clearInterval(timer); }; }, []);
  ...
  return <section className="flex h-auto flex-col overflow-hidden rounded-[1.45rem] border border-white/10 bg-white/[0.04] p-3 text-white shadow-[0_14px_38px_rgba(0,0,0,0.2)] backdrop-blur sm:rounded-3xl sm:p-4" aria-label="مباريات كأس العالم"><div className="flex flex-col gap-4"><div>{primaryMatch ? <MatchRow match={primaryMatch} now={now} variant={isConfirmedLive(primaryMatch) || isHalfTime(primaryMatch) ? 'live' : 'primary'} /> : <div className="rounded-2xl border border-dashed border-white/10 bg-black/20 p-4 text-sm font-bold text-gray-400">لا توجد مباراة رئيسية جاهزة للعرض الآن.</div>}</div><div><div className="mb-2 text-[10px] font-black uppercase tracking-[0.14em] text-gray-400">المباراتان القادمتان</div><div className="grid gap-3 md:grid-cols-2">{secondaryMatches.map((match) => <MatchRow key={matchKey(match)} match={match} now={now} />)}{!secondaryMatches.length ? <div className="rounded-2xl border border-dashed border-white/10 bg-black/20 p-4 text-sm font-bold text-gray-400 md:col-span-2">لا توجد مباريات قادمة إضافية جاهزة للعرض الآن.</div> : null}</div></div></div></section>;
}

export default function HomeClientSportsLiveFocus({ upcomingMatches = [], nextMarqueeMatch = null, playersCount = 0, teamsCount = 0, upcomingMatchesCount = 0 }: Props) {
  ...
  return <main dir="rtl" className="mx-auto max-w-7xl space-y-4 px-3 pb-8 pt-3 sm:space-y-6 sm:px-4 sm:py-5 lg:px-6"><div className="grid grid-cols-1 gap-4 lg:grid-cols-3 lg:items-start lg:gap-5"><div className="lg:col-span-2"><MatchCenter fallbackMatches={safeUpcomingMatches} nextMatch={safeNextMatch} /></div><div className="lg:col-span-1"><HomeGroupStandingsWidget compact /></div></div><HomeTournamentStatsCard playersCount={playersCount} teamsCount={teamsCount} upcomingMatchesCount={upcomingMatchesCount} /></main>;
}
