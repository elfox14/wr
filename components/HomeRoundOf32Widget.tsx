...
type Props = { knockoutMatches?: KnockoutMatch[] | unknown[] };
type Future = { no: number; from: [number, number] };
type DisplayTeam = { id?: string | null; name: string; code?: string | null; image?: string | null };
...
const R32 = [73, 75, 74, 77, 83, 84, 81, 82, 76, 78, 79, 80, 86, 88, 85, 87];
...
function displayTeamFromMatch(team?: TeamLite | null): DisplayTeam {
...
function extractNumber(value: unknown): number | null {
  ...
  if (typeof value === 'string') {
    ...
    const named = value.match(/(?:match|fixture|game|r32)[_\s-]*(7[3-9]|8\d|9\d|10[0-4])/i)?.[1];
...
function extractMatchNumber(match: KnockoutMatch) {
  const direct = extractNumber(match.externalId);
  ...
  while (stack.length) {
    ...
    if (n) return n;
    if (item && typeof item === 'object') Object.entries(item as Record<string, unknown>).forEach(([key, value]) => {
      if (/match.*number|matchno|match_no|fixture.*number/i.test(key)) stack.push(value);
  ...
  return (
    <div className={`flex h-8 items-center gap-1.5 rounded-md border px-1.5 ${winner ? 'border-[#FFD700]/45 bg-[#FFD700]/15' : 'border-white/10 bg-white/[0.035]'} ${reverse ? 'flex-row-reverse text-right' : 'text-left'}`}>
      <span className="flex h-6 w-9 shrink-0 items-center justify-center overflow-hidden rounded-md border border-white/20 bg-white">
        {flag ? <img src={flag} alt={`علم ${team.name}`} className="h-full w-full object-cover" loading="lazy" /> : <b className="text-[8px] text-black">{team.code || '—'}</b>}
      </span>
...
function R32MatchBlock({ no, side, match }: { no: number; side: 'left' | 'right'; match?: KnockoutMatch | null }) {
  const home = match ? displayTeamFromMatch(match.homeTeam) : emptyTeam(`FIFA ${no}`);
...
function SideBracket({ side, buckets, byNo }: { side: 'left' | 'right'; buckets: ReturnType<typeof groupMatches>; byNo: Map<number, KnockoutMatch> }) {
  ...
  const getR32Match = (no: number) => byNo.get(no) || null;
  const futureMatch = (no: number, list: Future[], bucket: KnockoutMatch[]) => byNo.get(no) || bucket[list.findIndex((match) => match.no === no)] || null;
  const r32Col = <div className="grid gap-2">{r32.map((no) => <R32MatchBlock key={no} no={no} side={side} match={getR32Match(no)} />)}</div>;
