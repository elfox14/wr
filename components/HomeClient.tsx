...
import { ArrowLeft, Calendar, Globe, Radio, ShieldCheck, Users, Wallet } from 'lucide-react';
...
function getAnimationHref(match: any) { const id = getAnimationMatchId(match); return id ? `/animation-live?matchId=${encodeURIComponent(String(id))}&lang=en&statsPanel=simple&teamPanel=1` : '/matches'; }
function isScoredMatch(match: any) { return ['IN_PLAY', 'LIVE', 'FINISHED'].includes(match?.status) || Number(match?.homeScore || 0) > 0 || Number(match?.awayScore || 0) > 0; }
function scoreLabel(match: any) { return `${Number(match?.homeScore || 0)} - ${Number(match?.awayScore || 0)}`; }
...
export default function HomeClient({ initialAssets, upcomingMatches = [], assetsCount = 0, playersCount = 0, teamsCount = 0, upcomingMatchesCount = 0, academyArticles = [] }: { initialAssets: any[]; usersCount?: number; tradeVolume?: number; executedTrades?: number; upcomingMatches?: any[]; assetsCount?: number; playersCount?: number; teamsCount?: number; upcomingMatchesCount?: number; recentTransactions?: any[]; mostTradedAssets?: any[]; topDemandAssets?: any[]; topMomentumAssets?: any[]; undervaluedAssets?: any[]; academyArticles?: AcademyArticle[] }) {
  ...
  const showScore = isScoredMatch(nextMatch);
  ...
  return <main className="mx-auto max-w-7xl space-y-7 px-4 py-7 sm:px-6 lg:px-8">
    <section className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_top_right,rgba(15,240,252,0.18),transparent_28%),radial-gradient(circle_at_bottom_left,rgba(255,215,0,0.12),transparent_24%),linear-gradient(135deg,rgba(255,255,255,0.055),rgba(255,255,255,0.015))] p-5 shadow-anti-gravity md:p-7">
      ...
      <div className="relative grid gap-5 lg:grid-cols-[1.02fr_0.98fr] lg:items-stretch"><div className="flex flex-col justify-center"><p className="mb-4 inline-flex w-fit rounded-full border border-[#0FF0FC]/25 bg-[#0FF0FC]/10 px-4 py-2 text-xs font-black text-[#0FF0FC]">بورصة إم سي للمونديال</p><h1 className="max-w-3xl text-3xl font-black leading-tight text-white md:text-5xl">منصة تحليلات وإحصائيات رياضية مع تجربة تداول افتراضية للمونديال</h1><p className="mt-4 max-w-3xl text-sm leading-7 text-gray-300 md:text-base">تابع بيانات المنتخبات واللاعبين، اقرأ التحليلات الكروية، وجرّب سوق بورصة المونديال بأرصدة افتراضية فقط لفهم حركة الأسعار والزخم بدون أي معاملات مالية حقيقية.</p></div>
        <div className="rounded-[1.6rem] border border-white/10 bg-black/40 p-5 backdrop-blur-xl"><div className="mb-4 flex items-center justify-between gap-3"><div><p className="text-xs font-black text-gray-500">المباراة القادمة</p><h2 className="mt-1 text-xl font-black text-white">{nextMatch ? (showScore ? 'النتيجة الآن' : 'مباراة قادمة') : 'جدول المباريات'}</h2></div><ShieldCheck className="text-[#0FF0FC]" size={30} /></div>
          {nextMatch ? <div className="rounded-2xl border border-white/10 bg-white/[0.045] p-4"><div className="mb-4 grid grid-cols-[1fr_auto_1fr] items-center gap-3"><div className="flex flex-col items-center gap-2 text-center">{renderMatchTeamLogo(nextMatch.homeTeam)}<Link href={getTeamHref(nextMatch.homeTeam)} className="text-xs font-black text-white transition hover:text-[#0FF0FC]">{nextMatch.homeTeam?.name || 'الفريق الأول'}</Link></div><div className="flex flex-col items-center gap-2"><Link href={groupHref(matchGroup)} className="rounded-full border border-[#0FF0FC]/25 bg-[#0FF0FC]/10 px-3 py-1 text-[10px] font-black text-[#0FF0FC] transition hover:bg-[#0FF0FC]/20 hover:text-white">المجموعة {matchGroup}</Link><div className={`${showScore ? 'rounded-2xl border border-emerald-400/25 bg-emerald-400/10 px-4 py-2 text-2xl font-black text-emerald-100' : 'rounded-full border border-white/10 bg-black/40 px-4 py-2 text-sm font-black text-[#FFD700]'}`}>{showScore ? scoreLabel(nextMatch) : 'VS'}</div></div><div className="flex flex-col items-center gap-2 text-center">{renderMatchTeamLogo(nextMatch.awayTeam)}<Link href={getTeamHref(nextMatch.awayTeam)} className="text-xs font-black text-white transition hover:text-[#0FF0FC]">{nextMatch.awayTeam?.name || 'الفريق الثاني'}</Link></div></div><div className="text-center text-xs font-bold text-gray-400">{new Date(nextMatch.matchDate).toLocaleString('ar-EG')}</div>{showScore ? <div className="mt-3 rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-3 py-2 text-center text-xs font-black text-emerald-200">النتيجة الحالية: {scoreLabel(nextMatch)}</div> : countdown ? <div className="mt-3 grid grid-cols-4 gap-2">{countdown.map((item) => <div key={item.label} className="rounded-xl border border-[#0FF0FC]/15 bg-[#0FF0FC]/[0.06] px-2 py-2 text-center"><div className="font-mono text-lg font-black leading-none text-white tabular-nums">{String(item.value).padStart(2, '0')}</div><div className="mt-1 text-[10px] font-bold text-[#0FF0FC]">{item.label}</div></div>)}</div> : <div className="mt-3 rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-3 py-2 text-center text-xs font-black text-emerald-200">بدأت أو اقتربت المباراة</div>}<Link href={animationHref} className={`mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl px-3 py-2 text-xs font-black transition ${hasAnimation ? 'border border-[#FFD700]/25 bg-[#FFD700]/10 text-[#FFD700] hover:bg-[#FFD700] hover:text-black' : 'border border-[#0FF0FC]/25 bg-[#0FF0FC]/10 text-[#0FF0FC] hover:bg-[#0FF0FC] hover:text-black'}`}><Radio size={15} /> {hasAnimation ? 'شاهد بث الأنيميشن' : 'المباريات القادمة'}</Link></div> : <div className="rounded-2xl border border-dashed border-white/10 bg-white/[0.03] p-6 text-center text-sm text-gray-400">سيظهر هنا أقرب لقاء بعد مزامنة جدول المباريات.</div>}
    ...
    {academyArticles.length > 0 && <section className="rounded-[2rem] border border-white/10 bg-surface p-5 shadow-card md:p-6"><div className="mb-5 flex items-center justify-between gap-3"><div><p className="text-xs font-black text-[#0FF0FC]">MC PRIME ACADEMY</p><h2 className="mt-1 text-2xl font-black text-white">أكاديمية بورصة المونديال</h2></div><Link href="/articles" className="rounded-2xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-black text-white hover:border-[#0FF0FC]/40 hover:text-[#0FF0FC]">كل المقالات</Link></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{academyArticles.map((article) => <Link key={article.id} href={`/article/${article.id}`} className="rounded-2xl border border-white/10 bg-black/20 p-4 transition hover:border-[#0FF0FC]/40 hover:bg-white/[0.06]"><div className="mb-3 text-[11px] font-black text-[#FFD700]">{article.category}</div><h3 className="line-clamp-2 font-black text-white">{article.title}</h3><p className="mt-2 line-clamp-3 text-xs leading-5 text-gray-400">{article.excerpt}</p></Link>)}</div></section>}
  </main>;
}
