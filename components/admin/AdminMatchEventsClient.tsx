...
export default function AdminMatchEventsClient() {
  ...
  function startEdit(item: MatchEventItem) {
    ...
    setForm({
      minute: item.minute === null || typeof item.minute === 'undefined' ? '' : String(item.minute),
      type: item.type || 'note',
      teamId: item.teamId || '',
      playerName: item.playerName || '',
      detail: item.detail || '',
      sourceName: item.sourceName || 'رصد يدوي',
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/admin/match-events', {
        ...
        body: JSON.stringify({ ...form, matchId: selectedMatchId, id: editingId }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'فشل حفظ الحدث');
      setMessage(editingId ? 'تم تعديل الحدث بنجاح.' : 'تم حفظ الحدث وظهر في مركز المباراة.');
      setForm(initialForm);
      setEditingId(null);
    ...
    <main className="min-h-screen bg-background px-4 py-6 text-white sm:px-6 lg:px-8" dir="rtl">
      <section className="mx-auto max-w-6xl space-y-6">
        <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-5 shadow-card md:p-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm font-bold leading-7 text-gray-400">أضف أو عدّل أحداث المباراة لتظهر في Timeline داخل مركز المباراة.</p>
            </div>
        ...
        <form onSubmit={submit} className="grid gap-5 rounded-[2rem] border border-white/10 bg-white/[0.035] p-5 md:grid-cols-2 md:p-6">
          {editingId && <div className="md:col-span-2 flex items-center justify-between gap-3 rounded-2xl border border-[#FFD700]/20 bg-[#FFD700]/10 p-4 text-sm font-black text-[#FFD700]"><span>تعديل حدث موجود</span><button type="button" onClick={cancelEdit} className="inline-flex items-center gap-1 rounded-xl bg-black/25 px-3 py-2 text-xs"><X size={13} /> إلغاء</button></div>}
        ...
        <section className="rounded-[2rem] border border-white/10 bg-white/[0.035] p-5 md:p-6">
          ...
          {loadingEvents ? <div className="p-8 text-center text-gray-500">جاري التحميل...</div> : events.length ? <div className="space-y-3">{events.map((item) => <article key={item.id} className="flex flex-col gap-3 rounded-2xl border border-white/8 bg-black/25 p-4 md:flex-row md:items-center md:justify-between"><div><div className="mb-1 flex flex-wrap items-center gap-2 text-[11px] font-black text-gray-500"><span>{item.minute ? `${item.minute}'` : '--'}</span><span>{item.type}</span>{item.sourceName && <span>{item.sourceName}</span>}</div>{item.playerName && <h3 className="font-black text-white">{item.playerName}</h3>}<p className="text-sm font-bold leading-6 text-gray-400">{item.detail}</p></div><div className="flex gap-2"><button type="button" onClick={() => startEdit(item)} className="inline-flex items-center justify-center gap-2 rounded-xl border border-[#0FF0FC]/20 bg-[#0FF0FC]/10 px-3 py-2 text-xs font-black text-[#0FF0FC] hover:bg-[#0FF0FC] hover:text-black"><Pencil size={14} /> تعديل</button><button type="button" onClick={() => deleteEvent(item.id)} className="inline-flex items-center justify-center gap-2 rounded-xl border border-red-400/20 bg-red-400/10 px-3 py-2 text-xs font-black text-red-300 hover:bg-red-400 hover:text-black"><Trash2 size={14} /> حذف</button></div></article>)}</div> : <div className="p-8 text-center text-gray-500">لا توجد أحداث لهذه المباراة بعد.</div>}
        </section>
      </section>
    </main>
  );
