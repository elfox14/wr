...
import { Activity, ArrowLeft, BarChart3, FileText, Loader2, Radio, RefreshCw, Save, Trash2, Video } from 'lucide-react';
...
type SnapshotItem = { id: string; provider: string; minute?: number | null; homeScore?: number | null; awayScore?: number | null; homePossession?: number | null; awayPossession?: number | null; homeShots?: number | null; awayShots?: number | null; homeShotsOnTarget?: number | null; awayShotsOnTarget?: number | null; xgHome?: number | null; xgAway?: number | null; sourceUrl?: string | null; capturedAt?: string };
type MatchDigestDraft = { matchId: string; matchTitle: string; scoreLine: string; statusLabel: string; summary: string; videoScript: string; facebookPost?: string | null; infographicPoints: string[]; status: string };
...
type ArticlePreview = { id: string; title: string; body: string };
...
export default function MatchDataConsole() {
  ...
  return <main className="min-h-screen bg-background px-4 py-6 text-white sm:px-6 lg:px-8" dir="rtl"><section className="mx-auto max-w-7xl space-y-6">
    ...
    <section className="grid gap-4 lg:grid-cols-[1.1fr_2fr]"><div className="rounded-[2rem] border border-white/10 bg-white/[0.035] p-5"><Field label="اختر المباراة"><select value={selectedMatchId} onChange={(event) => setSelectedMatchId(event.target.value)} className={inputClass}>{(data?.matches || []).map((match) => <option key={match.id} value={match.id}>{matchName(match)} — {formatDate(match.matchDate)}</option>)}</select></Field><button type="button" onClick={() => loadConsole(selectedMatchId)} className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-[#0FF0FC]/25 bg-[#0FF0FC]/10 px-4 py-3 text-sm font-black text-[#0FF0FC] hover:bg-[#0FF0FC] hover:text-black"><RefreshCw size={15} /> تحديث البيانات</button></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric title="المباراة" value={matchName(selectedMatch)} hint={formatDate(selectedMatch?.matchDate)} /><Metric title="النتيجة" value={scoreLine(selectedMatch, latestSnapshot)} hint={selectedMatch?.status || 'غير محدد'} /><Metric title="الأحداث" value={counts.total} hint={`${counts.goals} أهداف · ${counts.cards} بطاقات`} /><Metric title="Snapshots" value={counts.snapshots} hint={latestSnapshot?.provider || 'غير متوفر'} /></div></section>
