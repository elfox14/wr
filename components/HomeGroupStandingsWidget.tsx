...
function ThirdPlacedCard({ rows, compact }: { rows: ThirdPlaceRow[]; compact: boolean }) {
  return (
    <section className={`rounded-[1.2rem] border border-[#FFD700]/15 bg-black/25 ${compact ? 'p-2.5' : 'p-3'} shadow-[0_14px_38px_rgba(0,0,0,0.16)]`}>
      ...
      {rows.length ? (
        <div className="space-y-1.5">
          <div className="grid grid-cols-[minmax(0,1fr)_2rem_2rem_2rem] items-center gap-1.5 px-1.5 text-[8px] font-black text-gray-500">
            <span>المنتخب</span><span className="text-center">نقاط</span><span className="text-center">فارق</span><span className="text-center">أهداف</span>
          </div>
          <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-2">
            {rows.map((row) => {
              ...
              return (
                <Link key={`${row.groupKey}-${row.code}`} href={`/teams/${teamId}`} className="mobile-tap grid min-w-0 grid-cols-[auto_minmax(0,1fr)_2rem_2rem_2rem] items-center gap-1.5 rounded-xl border border-white/10 bg-white/[0.04] px-1.5 py-1.5 transition hover:border-[#0FF0FC]/35 hover:bg-white/[0.07]" title={`${row.team} - المجموعة ${formatCount(row.groupNumber)}`}>
                  <img src={flagUrl || undefined} alt="" className="h-3.5 w-5 shrink-0 rounded-[3px] object-cover" />
                  <span className="min-w-0"><span className="block truncate text-[10px] font-black text-white">{row.team}</span><span className="mt-0.5 block text-[8px] font-black text-gray-500">م{formatCount(row.groupNumber)}</span></span>
                  ...
                  <span className="rounded-md bg-white/[0.06] px-1 py-0.5 text-center text-[9px] font-bold text-gray-300">{formatGoalDifference(row.goalDifference)}</span>
      ...
      ) : <div className="rounded-xl border border-dashed border-white/10 bg-black/20 p-3 text-center text-[10px] font-bold text-gray-500">بيانات الثوالث غير متوفرة حالياً.</div>}
  ...
  return (
    <div className="flex h-auto flex-col gap-2">
      <section className={`flex h-auto flex-col rounded-[1.45rem] border border-white/10 bg-white/[0.04] ${sectionPadding} text-white shadow-[0_14px_38px_rgba(0,0,0,0.2)] backdrop-blur sm:rounded-3xl`}>
        {loading ? <div className="flex min-h-[12rem] items-center justify-center rounded-2xl border border-white/10 bg-black/20"><div className="h-6 w-6 animate-spin rounded-full border-2 border-[#0FF0FC] border-t-transparent" /></div>
          : groups.length === 0 ? <div className="flex items-center justify-center rounded-2xl border border-white/10 bg-black/20 p-4 text-center text-xs font-bold text-gray-500">جدول الترتيب غير متوفر حالياً.</div>
          : <div className="flex flex-col">
              <div className={`-mx-1 flex gap-1 overflow-x-auto px-1 scrollbar-none ${compact ? 'mb-2 pb-1' : 'mb-3 pb-2'}`}>
                {groups.map((group, index) => <button key={group.key} onClick={() => setSelectedGroupKey(group.key)} className={groupButtonClass(selectedGroupKey === group.key)} title={group.arName} aria-label={group.arName}>{formatCount(index + 1)}</button>)}
              </div>
              <div className="overflow-hidden rounded-2xl border border-white/10 bg-black/20">
                {selectedGroup ? <table className={`w-full text-right ${compact ? 'text-[10px]' : 'text-[11px] sm:text-xs'}`}>
                  <thead><tr className="border-b border-white/10 font-bold text-gray-400"><th className="w-7 px-1 py-1.5 text-center sm:w-8">#</th><th className="px-1 py-1.5">المنتخب</th><th className="w-7 px-1 py-1.5 text-center">لعب</th><th className="w-8 px-1 py-1.5 text-center">فارق</th><th className="w-8 px-1 py-1.5 text-center">ن</th></tr></thead>
                  <tbody>{selectedGroup.standings.map((row, index) => {
                    const flagUrl = getTeamFlagUrl({ code: row.code, name: row.team }, 40);
                    const teamId = `team-${row.code.toLowerCase()}`;
                    const isQualifying = index < 2;
                    return <tr key={row.code} className="border-b border-white/5 transition last:border-b-0 hover:bg-white/[0.02]">
                      <td className={`${cellPadding} text-center`}><span className={`inline-flex h-5 w-5 items-center justify-center rounded-md text-[10px] font-black ${isQualifying ? 'border border-[#00FF88]/20 bg-[#00FF88]/10 text-[#00FF88]' : index === 2 ? 'border border-[#FFD700]/20 bg-[#FFD700]/10 text-[#FFD700]' : 'bg-white/5 text-gray-400'}`}>{formatCount(index + 1)}</span></td>
                      <td className={`min-w-0 font-bold text-white ${cellPadding}`}><Link href={`/teams/${teamId}`} className="inline-flex min-w-0 max-w-full items-center gap-2 transition hover:text-[#0FF0FC]"><img src={flagUrl || undefined} alt="" className="h-4 w-5 shrink-0 rounded-sm object-cover" /><span className={compact ? 'max-w-[5rem] truncate' : 'max-w-[6.5rem] truncate sm:max-w-[10rem]'}>{row.team}</span></Link></td>
                      <td className={`${cellPadding} text-center font-medium text-gray-300`}>{formatCount(row.played)}</td>
                      <td className={`${cellPadding} text-center font-mono font-medium text-gray-300`}>{formatGoalDifference(row.goalDifference)}</td>
