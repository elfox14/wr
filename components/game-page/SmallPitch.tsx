...
type BenchSlot = { player: AnyPlayer; side: Side; index: number; total: number };
...
function makeSlots(players: AnyPlayer[], formation: string | null | undefined, side: Side): Slot[] { const starters = players.slice(0, 11); const rows = formationRows(formation, starters.length); const slots: Slot[] = []; let cursor = 0; const maxLine = Math.max(1, rows.length - 1); rows.forEach((count, line) => { const actual = Math.max(1, Math.min(count, starters.length - cursor)); for (let i = 0; i < actual; i += 1) { const player = starters[cursor++]; if (!player) continue; const x = side === 'home' ? 82 - (line / maxLine) * 34 : 18 + (line / maxLine) * 34; const y = actual === 1 ? 50 : 16 + (68 * i) / (actual - 1); slots.push({ player, side, x, y }); } }); return slots; }
...
function BenchDot({ slot }: { slot: BenchSlot }) { const n = numberOf(slot.player); const top = slot.total <= 1 ? 50 : 11 + (78 * slot.index) / Math.max(1, slot.total - 1); return <div className={`absolute z-30 flex -translate-y-1/2 items-center gap-1 rounded-full border border-white/15 bg-[#04110D]/88 p-1 shadow-xl backdrop-blur ${slot.side === 'away' ? 'left-1 flex-row sm:left-2' : 'right-1 flex-row-reverse sm:right-2'}`} style={{ top: `${top}%` }} title={`بديل: ${slot.player.name}`}><div className={`relative h-7 w-7 shrink-0 overflow-hidden rounded-full border ${slot.side === 'home' ? 'border-[#F8C846]' : 'border-[#18E58F]'}`}>{slot.player.image ? <img src={slot.player.image} alt={slot.player.name} className="h-full w-full object-cover" loading="lazy" /> : <span className="grid h-full place-items-center text-[9px] font-black text-white">{initials(slot.player.name)}</span>}{n ? <b className="absolute -bottom-0.5 -right-0.5 rounded-full bg-black px-1 text-[8px] text-white">{n}</b> : null}</div><span className="hidden max-w-[70px] truncate text-[9px] font-black text-white sm:block">{slot.player.name}</span></div>; }
function LineupsPanel({ data }: { data: MatchPageData }) { const o = data.officialLineup; const homeFormation = o?.home?.formation || '4-3-3'; const awayFormation = o?.away?.formation || '4-3-3'; const homeStarters = (o?.home?.startingXi?.length ? o.home.startingXi : data.homePlayers.slice(0, 11)).map((p) => enriched(p, data.homePlayers)); const awayStarters = (o?.away?.startingXi?.length ? o.away.startingXi : data.awayPlayers.slice(0, 11)).map((p) => enriched(p, data.awayPlayers)); const homeSubs = (o?.home?.substitutes?.length ? o.home.substitutes : data.homePlayers.slice(11)).map((p) => enriched(p, data.homePlayers)); const awaySubs = (o?.away?.substitutes?.length ? o.away.substitutes : data.awayPlayers.slice(11)).map((p) => enriched(p, data.awayPlayers)); const slots = [...makeSlots(awayStarters, awayFormation, 'away'), ...makeSlots(homeStarters, homeFormation, 'home')]; const benchSlots: BenchSlot[] = [...awaySubs.map((player, index) => ({ player, side: 'away' as Side, index, total: awaySubs.length })), ...homeSubs.map((player, index) => ({ player, side: 'home' as Side, index, total: homeSubs.length }))]; return <Section id="lineups" title="التشكيل الرسمي" icon={<Users size={22} />}><div className="rounded-[1.6rem] border border-[#18E58F]/25 bg-[#0b3b28] p-3"><div className="mb-3 flex justify-between gap-2 text-xs font-black"><span>{data.awayTeam.name} - {awayFormation}</span><span className="text-[#F8C846]">التشكيل والبدلاء داخل الملعب</span><span>{data.homeTeam.name} - {homeFormation}</span></div><div className="relative h-[640px] overflow-hidden rounded-[1.35rem] border-2 border-white/35 bg-[linear-gradient(90deg,rgba(255,255,255,.05)_0_50%,rgba(255,255,255,.08)_50%_100%)] sm:h-[700px] lg:h-[590px]"><FieldLines /><div className="absolute inset-x-3 top-3 flex justify-between text-[10px] font-black text-white/70"><span>بدلاء {data.awayTeam.name}</span><span>{o?.confirmed ? 'تشكيل مؤكد' : 'تشكيل محدث تلقائيًا'}</span><span>بدلاء {data.homeTeam.name}</span></div>{slots.length ? slots.map((slot, i) => <PlayerDot key={`${slot.side}-${slot.player.name}-${i}`} slot={slot} />) : <p className="absolute inset-0 grid place-items-center text-slate-300">التشكيل غير متوفر</p>}{benchSlots.map((slot) => <BenchDot key={`${slot.side}-bench-${slot.player.name}-${slot.index}`} slot={slot} />)}</div><p className="mt-3 text-center text-xs font-bold text-slate-300">اكتفينا بالملعب فقط: الأساسيون داخل الخطة، والبدلاء على طرفي الملعب. صور اللاعبين تُعرض من التشكيل الرسمي أو من قاعدة اللاعبين المحلية عند توفرها.</p></div></Section>; }
