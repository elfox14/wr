...
async function main() {
  console.log('Querying unverified finished matches from database...');
  let matches: any[] = [];
  ...
  for (let attempt = 1; attempt <= dbMaxAttempts; attempt++) {
    try {
      matches = await prisma.match.findMany({
        where: {
          status: { in: ['FINISHED', 'FT', 'AET', 'PEN', 'COMPLETED', 'ENDED', 'FULL_TIME'] }
        },
        select: {
          id: true,
          homeScore: true,
  ...
  console.log(`Found ${matches.length} unverified finished matches to force backfill.`);
  if (matches.length === 0) {
    console.log('All finished matches are already FINAL_VERIFIED!');
    return;
  }
