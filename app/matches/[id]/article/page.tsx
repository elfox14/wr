...
const GROUP_STAGE_WORDS = ['GROUP', 'group', 'المجموعة'];
...
type StatPair = { key: string; label: string; home: number | null; away: number | null; suffix?: string; higherIsBetter?: boolean };
...
type PlayerArticleStat = { id: string; name: string; teamName: string; teamCode?: string | null; image?: string | null; minutes: number; goals: number; assists: number; shots: number; shotsOnTarget: number; keyPasses: number; tackles: number; interceptions: number; saves: number; yellowCards: number; redCards: number; rating: number | null };
type SnapshotLike = { homePossession?: number | null; awayPossession?: number | null; homeShots?: number | null; awayShots?: number | null; homeShotsOnTarget?: number | null; awayShotsOnTarget?: number | null; homeShotsOffTarget?: number | null; awayShotsOffTarget?: number | null; homeCorners?: number | null; awayCorners?: number | null; homeYellowCards?: number | null; awayYellowCards?: number | null; homeRedCards?: number | null; awayRedCards?: number | null; homeAttacks?: number | null; awayAttacks?: number | null; homeDangerousAttacks?: number | null; awayDangerousAttacks?: number | null; capturedAt?: Date | string | null; providerMatchId?: number | null; rawData?: unknown };
type GroupContext = { label: string; before: StandingRow[]; after: StandingRow[]; homeBefore?: StandingRow; awayBefore?: StandingRow; homeAfter?: StandingRow; awayAfter?: StandingRow } | null;
...
function stageLabel(match: { stage?: string | null; groupPhase?: string | null }) { const raw = String(match.stage || match.groupPhase || '').toLowerCase(); if (raw.includes('32')) return 'دور الـ٣٢'; if (raw.includes('16')) return 'دور الـ١٦'; if (raw.includes('quarter')) return 'ربع النهائي'; if (raw.includes('semi')) return 'نصف النهائي'; if (raw.includes('final')) return 'النهائي'; if (match.groupPhase) return match.groupPhase.replace('GROUP_', 'المجموعة '); return 'دور المجموعات'; }
function isGroupStage(match: { stage?: string | null; groupPhase?: string | null }) { const text = `${match.stage || ''} ${match.groupPhase || ''}`; return Boolean(match.groupPhase) || GROUP_STAGE_WORDS.some((word) => text.includes(word)); }
...
function winner(match: { homeTeam: TeamLite; awayTeam: TeamLite; homeScore: number; awayScore: number }) { if (match.homeScore > match.awayScore) return { team: match.homeTeam, side: 'home' as const, goalsFor: match.homeScore, goalsAgainst: match.awayScore }; if (match.awayScore > match.homeScore) return { team: match.awayTeam, side: 'away' as const, goalsFor: match.awayScore, goalsAgainst: match.homeScore }; return null; }
function opponent(match: MatchLite, side: 'home' | 'away') { return side === 'home' ? match.awayTeam : match.homeTeam; }
function matchScore(match: { homeScore: number; awayScore: number }) { return `${fmt(match.homeScore)}-${fmt(match.awayScore)}`; }
function matchTitle(match: MatchLite, articleHeadline?: string) { return articleHeadline || `تحليل ${teamName(match.homeTeam)} و${teamName(match.awayTeam)}: ما وراء نتيجة ${matchScore(match)}`; }
...
function statPairs(snapshot: SnapshotLike | null): StatPair[] {
  ...
  return [
    ...
    { key: 'yellow', label: 'البطاقات الصفراء', home: snapshot.homeYellowCards ?? null, away: snapshot.awayYellowCards ?? null, higherIsBetter: false },
    { key: 'red', label: 'البطاقات الحمراء', home: snapshot.homeRedCards ?? null, away: snapshot.awayRedCards ?? null, higherIsBetter: false },
...
function metricLeader(metric: StatPair | null, match: MatchLite) { if (!metric || metric.home === null || metric.away === null || metric.home === metric.away) return null; const side = metric.home > metric.away ? 'home' : 'away'; return { side, team: side === 'home' ? match.homeTeam : match.awayTeam, value: side === 'home' ? metric.home : metric.away, diff: Math.abs(metric.home - metric.away) }; }
...
function movementText(before: StandingRow[] | undefined, after: StandingRow[] | undefined, teamId: string) { if (!before || !after) return ''; const b = rankOf(before, teamId); const a = rankOf(after, teamId); if (!a) return ''; if (!b) return `دخل جدول المجموعة في المركز ${fmt(a)} بعد هذه المباراة.`; if (a < b) return `تقدم من المركز ${fmt(b)} إلى المركز ${fmt(a)}.`; if (a > b) return `تراجع من المركز ${fmt(b)} إلى المركز ${fmt(a)}.`; return `حافظ على المركز ${fmt(a)}.`; }
function groupImpactText(group: GroupContext, match: MatchLite) {
  if (!group) return 'موقف المجموعة غير متوفر لأن المباراة لا ترتبط ببيانات مجموعة واضحة في قاعدة البيانات.';
  const home = movementText(group.before, group.after, match.homeTeamId);
  const away = movementText(group.before, group.after, match.awayTeamId);
  return `${teamName(match.homeTeam)}: ${home || standingLine(group.homeAfter)} ${teamName(match.awayTeam)}: ${away || standingLine(group.awayAfter)}`;
}
...
function importantEvents(events: any[]) { const prioritized = events.filter((event) => /goal|card|red|pen|var/i.test(`${event.type || ''} ${event.detail || ''}`)); return (prioritized.length ? prioritized : events).slice(0, 8); }
function turningPoint(events: any[], match: MatchLite) { const red = events.find((event) => /red|طرد|حمراء/i.test(`${event.type || ''} ${event.detail || ''}`)); if (red) return `لحظة الطرد عند الدقيقة ${fmt(red.minute)} غيّرت ميزان المباراة، لأنها أجبرت أحد الطرفين على تعديل تمركزه وإدارة المساحات بشكل مختلف.`; const goal = events.find((event) => /goal|هدف/i.test(`${event.type || ''} ${event.detail || ''}`)); if (goal) return `الهدف المسجل عند الدقيقة ${fmt(goal.minute)} كان نقطة التحول الأولى؛ بعده تغيرت حسابات الضغط، والمساحات، وإيقاع المخاطرة.`; const win = winner(match); if (win) return `نقطة التحول لم تظهر كحدث منفرد في البيانات، لكنها انعكست في قدرة ${teamName(win.team)} على تحويل النتيجة إلى أفضلية نهائية.`; return 'لا توجد لحظة تحول موثقة في الأحداث المتاحة، لذلك تبقى قراءة المباراة مرتبطة بسياق الأرقام العامة.'; }
...
function styleOfMatch(match: MatchLite, pairs: StatPair[]) {
  ...
  if (!win) {
    const leader = metricLeader(onTarget || shots || possession, match);
    return leader ? `التعادل لا يلغي أن ${teamName(leader.team)} كان الطرف الأكثر ظهورًا في ${onTarget ? 'التهديد المباشر' : shots ? 'المحاولات' : 'الاستحواذ'}، لكن غياب الحسم أبقى النتيجة متوازنة.` : 'المباراة جاءت متوازنة رقميًا، والتعادل يبدو أقرب لانعكاس عام لما توفر من بيانات.';
  }
  const side = win.side;
  const poss = sideValue(possession, side);
  const oppPoss = sideValue(possession, side === 'home' ? 'away' : 'home');
  const s = sideValue(shots, side);
  const oppS = sideValue(shots, side === 'home' ? 'away' : 'home');
  const ot = sideValue(onTarget, side);
  const oppOt = sideValue(onTarget, side === 'home' ? 'away' : 'home');
  ...
  if (conv !== null && conv >= 25) return `${teamName(win.team)} صنع الفارق بالفاعلية: ${fmt(win.goalsFor)} أهداف من ${fmt(s)} تسديدة، بنسبة تحويل تقارب ${oneDecimal.format(conv)}%.`;
  return `${teamName(win.team)} عرف كيف يدير تفاصيل المباراة حتى عندما لم تكن كل المؤشرات الرقمية تميل له بشكل واضح.`;
}
function analyticalLead(match: MatchLite, pairs: StatPair[], group: GroupContext, topPlayer: PlayerArticleStat | null) {
  const win = winner(match);
  const score = matchScore(match);
  const titleTeam = win ? teamName(win.team) : `${teamName(match.homeTeam)} و${teamName(match.awayTeam)}`;
  const groupPhrase = group ? ` وغيّر معه شكل الحسابات في ${group.label}` : '';
  const playerPhrase = topPlayer ? `، بينما جاء التأثير الفردي الأبرز من ${topPlayer.name}` : '';
  return win
    ? `${titleTeam} خرج من المباراة بانتصار ${score}${groupPhrase}${playerPhrase}. لكن القصة لا تقف عند النتيجة؛ الأهم هو كيف جاءت الأفضلية، وأين ترجمت الأرقام إلى لحظات حاسمة.`
    : `تعادل ${teamName(match.homeTeam)} و${teamName(match.awayTeam)} بنتيجة ${score}${groupPhrase}${playerPhrase}. قراءة المباراة هنا تحتاج إلى فصل واضح بين السيطرة الشكلية، جودة الفرص، وتأثير اللاعبين في اللحظات الحاسمة.`;
}
function articleHeadline(match: MatchLite, pairs: StatPair[], group: GroupContext) {
  const win = winner(match);
  const style = styleOfMatch(match, pairs);
  if (win) return `${teamName(win.team)} يحسم ${teamName(opponent(match, win.side))}: ${style.split('؛')[0]}`;
  if (group) return `تعادل يترك حسابات ${group.label} مفتوحة بين ${teamName(match.homeTeam)} و${teamName(match.awayTeam)}`;
  return `تعادل ${teamName(match.homeTeam)} و${teamName(match.awayTeam)}: أرقام تبحث عن حسم`;
}
...
async function loadArticle(matchId: string) {
  ...
  const groupMatchesPromise = typedMatch.groupPhase
    ? prisma.match.findMany({ where: { groupPhase: typedMatch.groupPhase, matchDate: { lte: typedMatch.matchDate }, status: { in: FINISHED } }, include: { homeTeam: { select: { id: true, name: true, code: true, image: true } }, awayTeam: { select: { id: true, name: true, code: true, image: true } } }, orderBy: { matchDate: 'asc' } })
    : Promise.resolve([]);
  ...
  const groupContext: GroupContext = typedMatch.groupPhase && after.length ? { label: typedMatch.groupPhase.replace('GROUP_', 'المجموعة '), before, after, homeBefore: before.find((row) => row.teamId === typedMatch.homeTeamId), awayBefore: before.find((row) => row.teamId === typedMatch.awayTeamId), homeAfter: after.find((row) => row.teamId === typedMatch.homeTeamId), awayAfter: after.find((row) => row.teamId === typedMatch.awayTeamId) } : null;
  const performancePlayers: PlayerArticleStat[] = playerPerformances.map((row) => ({ id: row.asset.id, name: row.asset.name, teamName: row.teamName || (row.asset.team ? teamName(row.asset.team) : 'غير متوفر'), teamCode: row.asset.team?.code || null, image: row.asset.image, minutes: num(row.minutes), goals: num(row.goals), assists: num(row.assists), shots: num(row.shotsTotal), shotsOnTarget: num(row.shotsOnTarget), keyPasses: num(row.keyPasses), tackles: num(row.tackles), interceptions: num(row.interceptions), saves: num(row.saves), yellowCards: num(row.yellowCards), redCards: num(row.redCards), rating: Number.isFinite(Number(row.apiRating ?? row.internalRating)) ? Number(row.apiRating ?? row.internalRating) : null }));
  ...
  return { match: typedMatch, events, latestSnapshot: latestSnapshot as SnapshotLike | null, players, rankedPlayers, pairs, groupContext };
}
...
function MetricCard({ pair, homeTeam, awayTeam }: { pair: StatPair; homeTeam: TeamLite; awayTeam: TeamLite }) { const home = pair.home ?? 0; const away = pair.away ?? 0; const total = Math.max(1, home + away); const homeWidth = Math.max(8, Math.min(92, (home / total) * 100)); return <article className="rounded-2xl border border-white/10 bg-black/25 p-3"><div className="mb-2 grid grid-cols-[68px_1fr_68px] items-center gap-3 text-center"><b className="text-base font-black text-[#FFD700]">{fmt(pair.home, pair.suffix || '')}</b><span className="text-xs font-black text-white">{pair.label}</span><b className="text-base font-black text-[#18E58F]">{fmt(pair.away, pair.suffix || '')}</b></div><div className="h-2 overflow-hidden rounded-full bg-white/10" dir="ltr"><span className="block h-full rounded-full bg-[#FFD700]" style={{ width: `${homeWidth}%` }} /></div><div className="mt-2 grid grid-cols-2 text-[10px] font-bold text-gray-500"><span>{teamName(homeTeam)}</span><span className="text-left">{teamName(awayTeam)}</span></div></article>; }
