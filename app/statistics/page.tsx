...
function upper(value?: string | null) { return String(value || '').trim().toUpperCase(); }
...
async function loadStatistics() {
  const [matches, latestSnapshotsRaw, performances, teamsCount, playersCount] = await Promise.all([
    ...
    prisma.playerPerformance.findMany({
      select: {
        ...
        internalRating: true,
        updatedAt: true,
        asset: { select: { id: true, name: true, code: true, image: true, team: { select: { name: true, code: true, image: true } } } },
      },
    }),
  ...
  for (const snapshot of latestSnapshotsRaw) if (!latestSnapshots.has(snapshot.matchId)) latestSnapshots.set(snapshot.matchId, snapshot);
  ...
  const scoredMatches = [...finishedMatches, ...liveMatches];
  ...
  for (const match of scoredMatches) {
    ...
    addSnapshotStats(teamRow(match.homeTeam), { shots: safe(snapshot.homeShots), shotsOnTarget: safe(snapshot.homeShotsOnTarget), corners: safe(snapshot.homeCorners), yellow: safe(snapshot.homeYellowCards), red: safe(snapshot.homeRedCards), possession: snapshot.homePossession });
    addSnapshotStats(teamRow(match.awayTeam), { shots: safe(snapshot.awayShots), shotsOnTarget: safe(snapshot.awayShotsOnTarget), corners: safe(snapshot.awayCorners), yellow: safe(snapshot.awayYellowCards), red: safe(snapshot.awayRedCards), possession: snapshot.awayPossession });
  }
  ...
  const totalGoals = scoredMatches.reduce((sum, match) => sum + match.totalGoals, 0);
  const finishedGoals = finishedMatches.reduce((sum, match) => sum + match.totalGoals, 0);
  const liveGoals = liveMatches.reduce((sum, match) => sum + match.totalGoals, 0);
  ...
  const updatedAt = latestSnapshotsRaw[0]?.capturedAt || performances[0]?.updatedAt || latestMatchSync || new Date();
  ...
  return {
    ...
    finishedGoals,
    liveGoals,
    averageGoals: scoredMatches.length ? totalGoals / scoredMatches.length : null,
    totalShots,
    totalShotsOnTarget,
    ...
    liveMatches: [...liveMatches].sort((a, b) => a.date.getTime() - b.date.getTime()).slice(0, 8),
  };
}
...
function PlayerRanking({ title, players, metricLabel, value }: { title: string; players: PlayerStats[]; metricLabel: string; value: (player: PlayerStats) => number | null }) {
  return <article className="rounded-3xl border border-white/10 bg-white/[0.035] p-3"><h3 className="text-sm font-black text-white">{title}</h3><div className="mt-3 space-y-2">{players.slice(0, 6).map((player, index) => <div key={player.id} className="flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-black/20 p-2"><div className="flex min-w-0 items-center gap-2"><span className="w-5 text-center text-[10px] font-black text-gray-500">{nf.format(index + 1)}</span><PlayerIdentity player={player} /></div><div className="shrink-0 rounded-xl border border-[#FFD700]/20 bg-[#FFD700]/10 px-2 py-1 text-center text-[10px] font-black text-[#FFD700]"><b className="block text-base leading-none">{num(value(player))}</b><span>{metricLabel}</span></div></div>)}</div></article>;
}
...
export default async function StatisticsPage() {
  ...
  const countedMatches = data.finishedMatches + data.liveMatches;
    ...
    <main dir="rtl" className="mx-auto max-w-7xl space-y-7 px-3 py-5 text-white sm:px-4 lg:px-6">
      <section className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_top_right,rgba(15,240,252,0.16),transparent_32%),linear-gradient(135deg,#061313,#050505)] p-5 shadow-[0_24px_70px_rgba(0,0,0,0.32)]">
        ...
        <div className="relative z-10 flex flex-wrap items-end justify-between gap-5">
          <div><p className="inline-flex rounded-full border border-[#FFD700]/25 bg-[#FFD700]/10 px-3 py-1 text-[10px] font-black tracking-[0.16em] text-[#FFD700]">LIVE DATA CENTER</p><h1 className="mt-4 text-3xl font-black leading-tight md:text-5xl">إحصائيات كأس العالم 2026</h1><p className="mt-3 max-w-3xl text-sm font-bold leading-7 text-gray-400">لوحة محدثة مباشرة من قاعدة البيانات: النتائج، المباريات الحية، أرقام المنتخبات، اللاعبين، والتسديدات. ركلات الترجيح تُحسب في تحديد الفائز داخل الأدوار الإقصائية.</p></div>
      ...
      <section id="overview" className="scroll-mt-24 space-y-4"><SectionTitle id="overview-title" kicker="OVERVIEW" title="نظرة عامة محدثة" description="الأهداف هنا تشمل المباريات المنتهية والمباشرة، بينما الفوز والتعادل والخسارة في جدول المنتخبات تُحسب للمباريات المنتهية فقط." /><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><StatCard label="المباريات المنتهية" value={`${num(data.finishedMatches)} / ${num(data.totalMatches)}`} note={`${num(data.liveMatches)} مباشر · ${num(data.scheduledMatches)} قادمة`} tone="gold" /><StatCard label="إجمالي الأهداف" value={num(data.totalGoals)} note={`${num(data.finishedGoals)} مكتملة · ${num(data.liveGoals)} مباشرة`} tone="green" /><StatCard label="إجمالي التسديدات" value={num(data.totalShots)} note={`${percent(data.shotAccuracy)} دقة على المرمى`} tone="cyan" /><StatCard label="مباريات لها نتيجة" value={num(countedMatches)} note={`${decimal(data.averageGoals)} هدف في المباراة`} /></div><div className="grid gap-3 md:grid-cols-3"><FeatureTeamCard title="أقوى هجوم" team={data.topAttack} metric={topAttackMetric} /><FeatureTeamCard title="أفضل دفاع" team={data.bestDefense} metric={bestDefenseMetric} /><FeatureTeamCard title="الأكثر استحواذًا" team={data.bestPossessionTeam} metric={possessionMetric} /></div></section>

      <section id="live" className="scroll-mt-24 space-y-4"><SectionTitle id="live-title" kicker="LIVE" title="المباريات المباشرة الآن" description="أي نتيجة مباشرة محفوظة في قاعدة البيانات تظهر هنا فورًا، حتى لا تختلف صفحة الإحصائيات عن شريط المباريات أو كارت البث." /><MatchTable matches={data.liveMatches} empty="لا توجد مباريات مباشرة الآن" /></section>

      <section className="space-y-4"><SectionTitle id="teams" kicker="TEAMS" title="ترتيب المنتخبات بالأرقام" description="جدول يجمع نتائج المنتخبات في المباريات المكتملة، مع إضافة الأهداف الحية الحالية إلى أعمدة له/عليه فقط." /><TeamTable teams={data.teams} /></section>

      <section className="space-y-4"><SectionTitle id="players" kicker="PLAYERS" title="أبرز اللاعبين" description="قوائم مختصرة لأهم اللاعبين حسب التسجيل، الصناعة، التسديد، صناعة الفرص، التصديات والانضباط." /><div className="grid gap-3 lg:grid-cols-2"><PlayerRanking title="الهدافون" players={data.topScorers} metricLabel="هدف" value={(player) => player.goals} /><PlayerRanking title="صناعة الأهداف" players={data.topAssists} metricLabel="أسيست" value={(player) => player.assists} /><PlayerRanking title="الأكثر تسديدًا" players={data.topShooters} metricLabel="تسديدة" value={(player) => player.shots} /><PlayerRanking title="صناعة الفرص" players={data.topCreators} metricLabel="تمريرة مفتاحية" value={(player) => player.keyPasses} /><PlayerRanking title="الحراس" players={data.topSaves} metricLabel="تصدي" value={(player) => player.saves} /><PlayerRanking title="الأعلى تقييمًا" players={[...data.players].sort((a, b) => (playerRating(b) || 0) - (playerRating(a) || 0)).slice(0, 8)} metricLabel="تقييم" value={(player) => playerRating(player)} /></div></section>
