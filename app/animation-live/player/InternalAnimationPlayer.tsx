        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="order-5 rounded-3xl border border-[#FFD700]/20 bg-[#FFD700]/[0.04] p-4">
            ...
            <div className="mb-3 rounded-2xl border border-white/10 bg-black/25 p-3">
              <div className="mb-3 grid grid-cols-[58px_1fr_84px_54px] items-center gap-2 text-[10px] font-black text-gray-500"><span>الفترة</span><span>القوة</span><span>الاتجاه</span><span>المؤشر</span></div>
              <div className="space-y-2">{momentumSegments.map((segment) => {
                ...
                return <button key={segment.key} type="button" disabled={!segment.topEvent} onClick={() => segment.topEvent ? selectEvent(segment.topEvent.id) : undefined} className="grid w-full grid-cols-[58px_1fr_84px_54px] items-center gap-2 text-right text-[10px] font-black text-gray-400 disabled:cursor-not-allowed"><span>د {segment.label}</span><span className="h-3 overflow-hidden rounded-full bg-white/10"><span className={`block h-full rounded-full ${segment.available ? 'bg-[#FFD700]' : 'bg-white/10'}`} style={{ width: `${width}%` }} /></span><span className="truncate text-[#FFD700]">{segment.available ? sideName(segment.leader, match?.homeTeam, match?.awayTeam) : '—'}</span><span className="text-white">{segment.available ? ar(score) : '—'}</span></button>;
              })}</div>
            </div>
