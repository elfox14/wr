...
function TeamMiniLogo({ image, name }: { image?: string | null; name: string }) {
  return (
    <div className="mx-auto mb-2 flex h-12 w-12 items-center justify-center overflow-hidden rounded-full bg-black/40 text-2xl">
      ...
      <section className="mx-auto max-w-7xl space-y-2 sm:space-y-3">
        ...
        {iframeUrl ? (
          ...
          <div className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-4 shadow-card md:p-5">
            ...
            {upcomingMatches.length > 0 ? <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{upcomingMatches.map((match) => <Link key={match.id} href={animationHref(match.animationMatchId)} className={`rounded-2xl border p-4 transition ${match.animationMatchId ? 'border-[#FFD700]/25 bg-[#FFD700]/[0.06] hover:bg-[#FFD700]/10' : 'border-white/10 bg-black/25 hover:border-[#0FF0FC]/30'}`}><div className="mb-3 flex items-center justify-between gap-3 text-[11px] font-black"><span className="rounded-full border border-white/10 bg-black/25 px-2 py-1 text-gray-300">{match.groupPhase || 'دور المجموعات'}</span><span className={match.animationMatchId ? 'text-[#FFD700]' : 'text-gray-500'}>{match.animationMatchId ? 'بث متاح' : 'بانتظار Match ID'}</span></div><div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2 text-center"><div><TeamMiniLogo image={match.homeTeam.image} name={match.homeTeam.name} /><div className="line-clamp-1 text-sm font-black text-white">{match.homeTeam.name}</div></div><div className="rounded-full border border-white/10 bg-black/40 px-3 py-1 text-xs font-black text-[#0FF0FC]">VS</div><div><TeamMiniLogo image={match.awayTeam.image} name={match.awayTeam.name} /><div className="line-clamp-1 text-sm font-black text-white">{match.awayTeam.name}</div></div></div><div className="mt-3 flex items-center justify-center gap-2 text-xs font-bold text-gray-400"><CalendarDays size={13} /> {new Date(match.matchDate).toLocaleString('ar-EG')}</div></Link>)}</div> : <div className="rounded-2xl border border-dashed border-white/10 bg-black/25 p-8 text-center text-sm text-gray-400">لا توجد مباريات قادمة حاليًا.</div>}
          </div>
        )}
