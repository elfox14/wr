...
const demoUrl = 'https://www.isportslive8.com/';
...
export default async function AnimationLivePage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const matchUrl = matchId && accessKey ? new URL('https://www.isportslive8.com/football/detail.html') : null;
  if (matchUrl) {
    matchUrl.searchParams.set('matchId', matchId);
    matchUrl.searchParams.set('accessKey', accessKey);
    matchUrl.searchParams.set('lang', lang);
    matchUrl.searchParams.set('statsPanel', statsPanel);
    if (teamPanel) matchUrl.searchParams.set('teamPanel', teamPanel);
  }

  const iframeUrl = matchUrl || new URL(demoUrl);
  const isDemo = !matchUrl;
  const notice = !matchId
    ? 'لا يوجد Match ID مربوط في قاعدة البيانات حاليًا، لذلك يتم عرض ديمو iSports Live Animation العام.'
    : !accessKey
      ? 'مفتاح ISPORTS_ANIMATION_ACCESS_KEY غير مضبوط على السيرفر، لذلك يتم عرض ديمو iSports Live Animation العام.'
      : '';
  ...
  return (
    <main className="min-h-screen bg-background px-2 py-2 text-white sm:px-4 lg:px-8">
      <section className="mx-auto max-w-7xl space-y-2 sm:space-y-3">
        <div className="rounded-2xl border border-white/10 bg-[radial-gradient(circle_at_top_right,rgba(15,240,252,0.13),transparent_30%),linear-gradient(135deg,rgba(255,255,255,0.04),rgba(255,255,255,0.01))] p-3 shadow-card md:p-4">
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              {isDemo && notice && <p className="mt-2 rounded-xl border border-[#FFD700]/20 bg-[#FFD700]/10 px-3 py-2 text-xs font-bold leading-5 text-[#FFD700]">{notice}</p>}
            </div>
        ...
        <div className="rounded-xl border border-[#FFD700]/20 bg-[#FFD700]/10 px-3 py-2 text-[11px] font-bold leading-5 text-[#FFD700] sm:hidden"><span className="inline-flex items-center gap-1"><RotateCcw size={13} /> يفضّل تدوير الهاتف أفقيًا أو الضغط على تكبير الشاشة.</span></div>
        <div className="overflow-hidden rounded-2xl border border-white/10 bg-black shadow-[0_25px_90px_rgba(0,0,0,0.45)]"><div className="flex flex-wrap items-center justify-between gap-2 border-b border-white/10 bg-white/[0.035] px-3 py-2 text-[10px] font-black text-gray-300 md:px-4"><span>{isDemo ? 'iSports Demo Mode' : `Match ID: ${matchId}`}</span><span>Language: {lang}</span>{!isDemo && <span>Stats: {statsPanel}</span>}{!isDemo && <span>Team Panel: {teamPanel || 'default'}</span>}</div><iframe title="Football Animation Live" src={iframeUrl.toString()} className="h-[82vh] w-full border-0 bg-black sm:h-[80vh] lg:h-[78vh]" allow="fullscreen; autoplay; encrypted-media; picture-in-picture" allowFullScreen referrerPolicy="strict-origin-when-cross-origin" /></div>
        {isDemo && (availableMatches.length > 0 || waitingMatches.length > 0) && <div className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-4 shadow-card md:p-5"><div className="mb-4 flex flex-col gap-2 md:flex-row md:items-center md:justify-between"><div><p className="text-xs font-black text-[#0FF0FC]">ربط المباريات</p><h2 className="mt-1 text-2xl font-black text-white">المباريات الموجودة في جدول المنصة</h2></div><Link href="/matches" className="inline-flex items-center justify-center rounded-2xl border border-white/10 bg-white/5 px-4 py-2 text-xs font-black text-white hover:border-[#0FF0FC]/40 hover:text-[#0FF0FC]">كل المباريات</Link></div>{availableMatches.length > 0 && <><h3 className="mb-3 text-sm font-black text-[#FFD700]">بث أنيميشن متاح</h3><div className="mb-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{availableMatches.map((match) => <MatchChoiceCard key={match.id} match={match} />)}</div></>}{waitingMatches.length > 0 && <><h3 className="mb-3 text-sm font-black text-gray-400">بانتظار الربط</h3><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{waitingMatches.map((match) => <MatchChoiceCard key={match.id} match={match} />)}</div></>}</div>}
