  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setJoinCode('');
        router.push(`/groups/${data.leagueId}`);
      } else {
        const err = await res.json();
        toast.error(err.error || 'حدث خطأ');
      }
      ...
      <main className="max-w-6xl mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
          ...
          <div className="bg-[#1A1A1A] border border-white/10 rounded-3xl p-8 hover:border-[#CD7F32]/30 transition-all">
            ...
            <h2 className="text-2xl font-bold mb-2">الانضمام إلى مجموعة</h2>
        ...
        {/* User Leagues */}
        <div>
          <h2 className="text-3xl font-bold mb-6 border-b border-white/10 pb-4">مجموعاتي ({leagues.length})</h2>
          ...
          {leagues.length === 0 ? (
            <div className="text-center py-12 bg-[#1A1A1A] border border-white/5 rounded-3xl">
              ...
              <p className="text-gray-400 text-lg">لم تنضم لأي مجموعة بعد.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {leagues.map(league => (
                <Link href={`/groups/${league.id}`} key={league.id}>
