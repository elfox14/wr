...
const LIVE_STAGE_TIMEOUT_MS = 18_000;
...
function bool(value: string | null | undefined, fallback = true) {
...
function buildFootballDataUrl(origin: string, key: string, url: URL) {
  ...
  next.searchParams.set('daysBack', String(int(url.searchParams.get('daysBack'), 1, 0, 3)));
  next.searchParams.set('daysAhead', String(int(url.searchParams.get('daysAhead'), 1, 0, 3)));
...
function buildFullSyncUrl(origin: string, key: string, url: URL) {
  ...
  next.searchParams.set('lineups', 'false');
  next.searchParams.set('isports', 'false');
  next.searchParams.set('isportStats', 'false');
  next.searchParams.set('isportVisualStats', 'false');
  next.searchParams.set('dedupe', 'true');
  next.searchParams.set('wakeFallback', 'false');
  next.searchParams.set('mapISports', 'false');
  next.searchParams.set('limit', String(int(url.searchParams.get('limit'), 3, 1, 6)));
  next.searchParams.set('minutesBack', String(int(url.searchParams.get('minutesBack'), 180, 15, 360)));
  next.searchParams.set('minutesForward', String(int(url.searchParams.get('minutesForward'), 180, 0, 360)));
  return withSecret(next, key);
}
...
function buildISportsTimelineUrl(origin: string, key: string, url: URL, target: { dbMatchId?: string | null; providerMatchId?: string | null }) {
  ...
  next.searchParams.set('asyncLive', 'false');
  next.searchParams.set('timeoutMs', String(int(url.searchParams.get('isportsTimeoutMs'), 12_000, 5_000, 25_000)));
  next.searchParams.set('waitMs', String(int(url.searchParams.get('isportsWaitMs'), 2_000, 500, 8_000)));
...
export async function GET(req: Request) {
  ...
  const budgetMs = int(url.searchParams.get('budgetMs'), LIVE_STAGE_TIMEOUT_MS, 5_000, 30_000);
  ...
  const heavyISportsAllowed = bool(url.searchParams.get('allowHeavyISports'), bool(process.env.ENABLE_HEAVY_ISPORTS_CRON, false));

  const stages: StageResult[] = [];

  if (bool(url.searchParams.get('footballData'), true)) {
    stages.push(await callJson('football_data_sync', buildFootballDataUrl(origin, key, url), 6_000, startedAt, budgetMs));
  }

  if (bool(url.searchParams.get('theStats'), true)) {
    stages.push(await callJson('the_stats_light_sync', buildFullSyncUrl(origin, key, url), 10_000, startedAt, budgetMs));
  }

  if (heavyISportsAllowed && bool(url.searchParams.get('isportsTimeline'), false)) {
    stages.push(await callJson('isports_timeline_manual_safe', buildISportsTimelineUrl(origin, key, url, target), 8_000, startedAt, budgetMs));
  } else {
    stages.push({ name: 'isports_timeline', ok: true, skipped: true, error: 'disabled_by_default_to_protect_render_memory' });
  }
  ...
  return json({
    ok: true,
    mode: 'worldcup_live_auto_memory_safe',
    authMode: auth.mode,
    origin,
    target,
    budgetMs,
    ...
    stages,
    note: 'Memory-safe default: heavy iSports browser/visual/background stages are disabled unless allowHeavyISports=true and isportsTimeline=true are explicitly passed.',
  });
}
