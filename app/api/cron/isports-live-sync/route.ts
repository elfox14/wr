...
function compactTimeline(result: any) {
  return {
    ...
    eventsPreview: Array.isArray(result?.timeline?.events) ? result.timeline.events.slice(0, 4) : [],
    error: result?.error || null,
  };
}
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const timeoutMs = clampInt(url.searchParams.get('timeoutMs'), 30000, 5000, 60000);
    const waitMs = clampInt(url.searchParams.get('waitMs'), 9000, 1000, 30000);
    ...
    for (const match of matches) {
      ...
      try {
        ...
        if (includeLive) {
          const liveUrl = new URL('/api/internal/live-ingest/isports/remote-live-pull', origin);
          ...
          const live = await callRoute(remoteLivePullGET, liveUrl, adminSecret);
