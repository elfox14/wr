...
async function run(req: Request) {
  ...
  for (const match of matches) {
    try {
      ...
      if (!dryRun) {
        ...
        const snapshot = await prisma.matchStatsSnapshot.create({ data: { id: randomUUID(), matchId: match.id, provider: 'THE_STATS_API_EXTRAS', providerMatchId: providerMatchNumber(providerMatchId), homeScore: match.homeScore, awayScore: match.awayScore, ...snapshotStatColumns(normalized), rawData: { provider: 'THE_STATS_API', mode: 'final_canonical_after_match_snapshot_only', endpointMode, importedAt: new Date().toISOString(), resolvedProviderMatchId: providerMatchId, resolvedBy: (collected as any).resolvedBy, rateLimitPolicy: { requestsPerMinute, delayMs, note: endpointMode === 'full' ? 'Full mode is explicit and should be used sparingly.' : 'Essential mode avoids heavy shotmap/player-stats endpoints by default.' }, displayPolicy: { eventsSource: 'snapshot.normalized.eventsDetailed.all', writeMatchEvents, fallbackMetrics: 'iSports snapshots may be kept for attacks/dangerous attacks only.' }, normalized } }, select: { id: true } });
        snapshotId = snapshot.id; insertedEvents = 0; wroteFinalSnapshots = true;
      }
      processed.push({ matchId: match.id, teams: `${match.homeTeam?.name || match.homeTeamId} vs ${match.awayTeam?.name || match.awayTeamId}`, status: dryRun ? 'dry_run_ok' : 'finalized_from_the_stats_snapshot_only', endpointMode, providerMatchId, resolvedBy: (collected as any).resolvedBy, counts: { stats: statsCount, events: events.length, shots: shots.length, playerStats: players.length, lineups: normalized?.lineups ? 1 : 0 }, deleted, snapshotId, insertedEvents, writeMatchEvents });
    } catch (error: any) {
  ...
  return NextResponse.json({ ok: true, mode: 'the_stats_finalize_matches_v3_essential_default', dryRun, endpointMode, note: dryRun ? 'Add apply=true or dryRun=false to write final canonical TheStats snapshot.' : 'Final TheStats snapshot was written. Essential mode is default to avoid provider 429 on heavy endpoints.', policy: { sourceOfTruth: 'THE_STATS_API for final post-match events and statistics', fullMode: 'Use endpointMode=full only manually when shotmap/player-stats are needed.', requestsPerMinute, delayMs, theStatsLimitSafety: requestsPerMinute <= 90 }, cleanup: { purgeISportsEvents, purgeISportsSnapshots, purgeFootballDataEvents, purgeTheStatsMatchEvents, replaceTheStatsFinal }, scope: { matchId, limit: matchId ? 1 : limit, days, localMatches: matches.length }, providerRequestsBudgetApprox, processed, revalidated }, { headers: { 'Cache-Control': 'no-store' } });
}
