...
async function autoFinishFromOfficialTimeline(match: any, result: any, dryRun: boolean) {
  ...
  const liveStatsOk = Boolean(result?.liveStatsOk);
  const conflictFinish = isNotLiveConflict(result) && providerEventsFound > 0 && latestMinute >= 90;
  const staleOverLimitFinish = latestMinute >= 120 && liveStatsOk;
  const shouldFinish = conflictFinish || staleOverLimitFinish;
  if (!shouldFinish) return { checked: true, shouldFinish: false, reason: 'official_timeline_does_not_confirm_finished', isNotLiveConflict: isNotLiveConflict(result), latestMinute, providerEventsFound, liveStatsOk };
  if (!dryRun && String(match.status || '').toUpperCase() !== 'FINISHED') await prisma.match.update({ where: { id: match.id }, data: { status: 'FINISHED' } });
  return { checked: true, shouldFinish: true, updated: !dryRun, reason: staleOverLimitFinish ? 'TheStats live minute reached 120+, so stale IN_PLAY clock was closed as FINISHED' : 'TheStats timeline exists, latestMinute >= 90, and live-stats says match is not live', latestMinute, providerEventsFound, liveStatsOk };
}
...
export async function GET(req: Request) {
  ...
  const out: any = {
    ok: true,
    mode: 'live_match_full_sync_priority',
    dryRun,
    requestedMatchId: requestedMatchId || null,
    publicOrigin: origin,
    matchesFound: 0,
    policy: {
      selection: 'LIVE and closest-to-now matches are prioritized before old scheduled/stale rows',
      theStats: 'primary after match: /timeline imports official events with skipSimilarExisting=false, then /stats imports final stats when available',
      fallback: 'iSports remains fallback only when TheStats timeline is unavailable',
  ...
  for (const [index, match] of matches.entries()) {
    ...
    const item: any = {
      ...
      priorityScore: Number(priorityScore(match, Date.now()).toFixed(4)),
      theStatsLive: null,
      theStatsPostmatchEvents: null,
      resolvedTheStatsMatchId: null,
      theStatsMatchInfo: null,
    ...
    } else if (runTheStatsPostmatch) item.theStatsPostmatchFinal = { skipped: true, reason: theStatsBlocked ? unavailableReason(item.theStatsLive) : 'Postmatch final is only requested after finished/not-live state' };
