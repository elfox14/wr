...
async function selectActiveMatches(matchId: string, minutesBack: number, minutesForward: number, postMatchMinutes: number, limit: number) { const now = Date.now(); const include = { homeTeam: { select: { name: true, code: true } }, awayTeam: { select: { name: true, code: true } } }; if (matchId) return prisma.match.findMany({ where: { id: matchId }, include, take: 1 }); const candidates = await prisma.match.findMany({ where: { OR: [{ status: { in: LIVE } }, { matchDate: { gte: new Date(now - minutesBack * 60_000), lte: new Date(now + minutesForward * 60_000) }, status: { notIn: FINISHED } }, { status: { in: FINISHED }, matchDate: { gte: new Date(now - postMatchMinutes * 60_000), lte: new Date(now) }, statsSnapshots: { none: { provider: "THE_STATS_API_EXTRAS" } } }] }, include, orderBy: { matchDate: 'asc' }, take: Math.max(20, limit * 4) }); return candidates.sort((a, b) => priorityScore(a, now) - priorityScore(b, now)).slice(0, limit); }
