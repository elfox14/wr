...
import { seedGroupAFbrefReports } from '@/lib/groupAFbrefStats';
...
const GROUP_A_CODES = ['MEX', 'ZAF', 'RSA', 'KOR', 'CZE'];
...
export async function POST(request: Request) {
  ...
  try {
    const groupATeams = await prisma.asset.findMany({
      where: {
        type: 'TEAM',
    ...
    const deleted = await prisma.teamIntelligenceReport.deleteMany({
      where: {
        ...
        teamId: { in: groupATeams.map((team) => team.id) },
      },
    });

    const seedResult = await seedTeamIntelligenceReports(prisma);
    const fbrefSeedResult = await seedGroupAFbrefReports(prisma);

    return NextResponse.json({
      success: true,
      deletedGroupAReports: deleted.count,
      groupATeams,
      curatedSeed: seedResult,
      fbrefSeed: fbrefSeedResult,
    });
  } catch (error) {
    console.error('Failed to reseed Group A intelligence reports:', error);
    return NextResponse.json({ error: 'Failed to reseed Group A intelligence reports' }, { status: 500 });
  }
}
...
export async function GET(request: Request) {
  ...
  return NextResponse.json({
    ok: true,
    message: 'Use POST to delete and reseed curated + FBref copied-source Group A intelligence reports.',
    groupACodes: GROUP_A_CODES,
  });
}
