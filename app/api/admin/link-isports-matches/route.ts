...
export async function GET(req: Request) {
  ...
  try {
    ...
    return NextResponse.json({ ok: false, error: error.message || 'Failed to fetch provider fixtures', provider: error.provider || null, details: error.payload || null }, { status: error.status || 500 });
  }
  ...
  for (const match of localMatches) {
    const ranked = fixtures
      .map((fixture) => ({ fixture, candidate: scoreCandidate(match, fixture) }))
      .filter((item) => Number.isFinite(item.fixture.fixture?.id))
      .sort((a, b) => b.candidate.finalScore - a.candidate.finalScore);
    const best = ranked[0];
    candidatesDebug.push({
      localMatchId: match.id,
      local: `${match.homeTeam.name} vs ${match.awayTeam.name}`,
      best: best ? {
        fixtureId: best.fixture.fixture?.id,
        provider: `${best.candidate.homeName} vs ${best.candidate.awayName}`,
        finalScore: best.candidate.finalScore,
        teamScore: best.candidate.teamScore,
        orientation: best.candidate.orientation,
        providerDate: best.candidate.providerDate,
      } : null,
    });
    ...
    if (!dryRun) {
      await prisma.match.update({
        ...
        data: {
          animationMatchId,
          status: best.fixture.fixture?.status?.short || match.status,
          homeScore: Number(best.fixture.goals?.home ?? match.homeScore),
          awayScore: Number(best.fixture.goals?.away ?? match.awayScore),
        },
      });
    }
    linked.push({
      ...
      animationMatchId,
      provider: `${best.candidate.homeName} vs ${best.candidate.awayName}`,
      score: best.candidate.finalScore,
      dryRun,
    });
  }
  ...
  return NextResponse.json({
    ...
    unmatched,
    candidatesDebug,
    nextAction: dryRun ? 'Review linked candidates, then rerun with dryRun=false to save animationMatchId.' : 'Saved animationMatchId. Rerun diagnostics and open /animation-live.',
  });
}
