...
function integerOrNull(value: unknown, min?: number, max?: number) {
  const numeric = numberOrNull(value, min, max);
...
async function ensureConsoleTables() {
  ...
  `);
  await prisma.$executeRawUnsafe('ALTER TABLE "MatchEvent" ADD COLUMN IF NOT EXISTS "playerId" TEXT');
  await prisma.$executeRawUnsafe('CREATE INDEX IF NOT EXISTS "MatchEvent_matchId_minute_idx" ON "MatchEvent" ("matchId", "minute")');
  await prisma.$executeRawUnsafe('CREATE INDEX IF NOT EXISTS "MatchEvent_playerId_idx" ON "MatchEvent" ("playerId")');
...
function matchTitle(match: any) {
  return `${teamName(match?.homeTeam, 'الفريق الأول')} ضد ${teamName(match?.awayTeam, 'الفريق الثاني')}`;
}

function ar(value: unknown) {
  if (value === null || value === undefined || value === '') return 'غير متوفر';
  const n = Number(value);
  if (Number.isFinite(n)) return new Intl.NumberFormat('ar-EG').format(n);
  return String(value);
}

function scoreLine(match: any, snapshot?: any) {
  const home = snapshot?.homeScore ?? match?.homeScore ?? 0;
  const away = snapshot?.awayScore ?? match?.awayScore ?? 0;
  return `${ar(home)} - ${ar(away)}`;
}
...
function publicSnapshot(row: any) {
  ...
  const sourceName = raw.sourceName || raw.providerName || row.provider;
  ...
  return {
    ...
    sourceName,
    sourceUrl,
    xgHome: xg?.home ?? xg?.local ?? null,
    xgAway: xg?.away ?? xg?.visitor ?? null,
  };
}
...
function buildBroadcastDigest(match: any, snapshot: any, events: ConsoleEvent[]) {
  ...
  const eventLines = importantEvents.length
    ? importantEvents.map((event) => `د${ar(event.minute)}: ${eventTypeLabel(event.type)} - ${eventSide(event, match)}${event.playerName ? ` (${event.playerName})` : ''}. ${event.detail}`)
    : ['الأحداث التفصيلية غير كافية حتى الآن، لذلك يعتمد النص على النتيجة والإحصائيات المحفوظة.'];
  const sourceNames = Array.from(new Set([
    snapshot?.sourceName || snapshot?.provider,
    ...events.map((event) => event.sourceName),
  ].filter(Boolean).map(String)));
  const sourceLine = sourceNames.length ? `مصادر المراجعة: ${sourceNames.join('، ')}.` : 'مصادر المراجعة: غير متوفر في المصادر.';

  const summary = [
    `ملخص ${title}: النتيجة الحالية/النهائية ${score}، والقراءة مبنية على أحداث المباراة والإحصائيات المحفوظة داخل قاعدة البيانات.`,
    stats.length ? `أبرز الأرقام: ${stats.join(' | ')}.` : 'الأرقام التفصيلية غير مكتملة حاليًا.',
    importantEvents[0] ? `نقطة البداية المهمة جاءت مع حدث الدقيقة ${ar(importantEvents[0].minute)}: ${importantEvents[0].detail}` : 'لا توجد نقطة تحول مؤكدة في الأحداث المحفوظة حتى الآن.',
  ].join('\n');

  const videoScript = [
    `أهلًا بكم في تحليل سريع لمباراة ${title}. [BREAK 1s]`,
    `النتيجة الآن ${score}، وهذه قراءة ${latestMinute} اعتمادًا على البيانات المحفوظة في Match Data Console. [BREAK 1s]`,
    `أولًا، تسلسل الأحداث المهمة:`,
    ...eventLines.map((line) => `${line} [BREAK 1s]`),
    stats.length ? `ثانيًا، قراءة الأرقام: ${stats.join('، ')}. [BREAK 1s]` : 'ثانيًا، الإحصائيات التفصيلية غير مكتملة، لذلك لا نخترع أرقامًا غير موجودة. [BREAK 1s]',
    `تكتيكيًا، ركّز على لحظتين: من كان أكثر قدرة على تحويل الضغط إلى فرص، ومن تعامل أفضل مع التحولات بعد فقدان الكرة. [BREAK 1s]`,
    `للنشر السريع، استخدم هذا السكريبت ليوتيوب شورتس أو تيك توك، ثم أضف لقطة للأحداث وجدولًا صغيرًا للأرقام. [BREAK 1s]`,
    sourceLine,
  ].join('\n\n');

  const infographicPoints = [
    `النتيجة: ${score}`,
    snapshot?.minute ? `آخر دقيقة محفوظة: ${ar(snapshot.minute)}` : 'آخر دقيقة محفوظة: غير متوفر',
    stats[0] || 'الاستحواذ: غير متوفر',
    stats[1] || 'التسديدات: غير متوفر',
    importantEvents[0] ? `أبرز حدث: د${ar(importantEvents[0].minute)} ${importantEvents[0].detail}` : 'أبرز حدث: غير متوفر',
  ];

  const facebookPost = [
    `⚽ ${title}`,
    `النتيجة: ${score}`,
    importantEvents[0] ? `أهم لقطة: د${ar(importantEvents[0].minute)} - ${importantEvents[0].detail}` : 'أهم لقطة: غير متوفر في الأحداث المحفوظة',
    stats.length ? `رقم سريع: ${stats[0]}` : 'الأرقام التفصيلية غير مكتملة بعد.',
    'ما تقييمك للمباراة؟',
  ].join('\n');
  ...
  return {
    ...
    matchTitle: title,
    scoreLine: score,
    statusLabel: match.status || 'غير محدد',
    summary,
    turningPoint: importantEvents[0]?.detail || null,
    videoScript,
    facebookPost,
    infographicPoints,
    status: 'draft',
...
async function loadConsole(matchId?: string | null) {
  ...
  const selectedMatch = selectedMatchId
    ? await prisma.match.findUnique({ where: { id: selectedMatchId }, include: { homeTeam: true, awayTeam: true } })
    : null;
  const events = selectedMatchId
    ? await prisma.matchEvent.findMany({ where: { matchId: selectedMatchId }, orderBy: [{ minute: 'asc' }, { createdAt: 'asc' }], take: 150 })
    : [];
  ...
  const digests = selectedMatchId
    ? await prisma.$queryRawUnsafe<any[]>(`SELECT * FROM "MatchDigest" WHERE "matchId" = ${quoteSql(selectedMatchId)} LIMIT 1`)
    : [];
  ...
  return {
    ...
    snapshotHistory: history.map(publicSnapshot),
    existingDigest: digests[0] || null,
  };
}

async function saveManualSnapshot(body: any) {
  ...
  const rawData = {
    sourceName: textOrNull(body.sourceName) || provider,
    sourceUrl: textOrNull(body.sourceUrl),
    reviewNote: textOrNull(body.reviewNote),
    xg: { home: numberOrNull(body.xgHome, 0), away: numberOrNull(body.xgAway, 0) },
    savedBy: 'match-data-console',
  };
  await prisma.matchStatsSnapshot.create({
    data: {
      ...
      provider,
      providerMatchId,
      minute: integerOrNull(body.minute, 0, 130),
      homePossession: integerOrNull(body.homePossession, 0, 100),
      awayPossession: integerOrNull(body.awayPossession, 0, 100),
      homeAttacks: integerOrNull(body.homeAttacks, 0),
      awayAttacks: integerOrNull(body.awayAttacks, 0),
      homeDangerousAttacks: integerOrNull(body.homeDangerousAttacks, 0),
      awayDangerousAttacks: integerOrNull(body.awayDangerousAttacks, 0),
      homeShots: integerOrNull(body.homeShots, 0),
      awayShots: integerOrNull(body.awayShots, 0),
      homeShotsOnTarget: integerOrNull(body.homeShotsOnTarget, 0),
      awayShotsOnTarget: integerOrNull(body.awayShotsOnTarget, 0),
      homeShotsOffTarget: integerOrNull(body.homeShotsOffTarget, 0),
      awayShotsOffTarget: integerOrNull(body.awayShotsOffTarget, 0),
      homeCorners: integerOrNull(body.homeCorners, 0),
      awayCorners: integerOrNull(body.awayCorners, 0),
      homeYellowCards: integerOrNull(body.homeYellowCards, 0),
      awayYellowCards: integerOrNull(body.awayYellowCards, 0),
      homeRedCards: integerOrNull(body.homeRedCards, 0),
      awayRedCards: integerOrNull(body.awayRedCards, 0),
      homeScore: integerOrNull(body.homeScore, 0),
      awayScore: integerOrNull(body.awayScore, 0),
      rawData,
    },
  });
...
export async function POST(req: Request) {
  ...
  if (action === 'save_stats_snapshot') return saveManualSnapshot(body);
