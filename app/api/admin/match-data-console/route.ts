...
function publicSnapshot(row: any) {
  ...
  return {
    ...
    sourceName: raw.sourceName || raw.providerName || row.provider,
...
function buildBroadcastDigest(match: any, snapshot: any, events: ConsoleEvent[]) {
  ...
  const facebookPost = [`⚽ ${title}`, `النتيجة: ${score}`, importantEvents[0] ? `أهم لقطة: د${ar(importantEvents[0].minute)} - ${importantEvents[0].detail}` : 'أهم لقطة: غير متوفر في الأحداث المحفوظة', stats.length ? `رقم سريع: ${stats[0]}` : 'الأرقام التفصيلية غير مكتملة بعد.', 'ما تقييمك للمباراة؟'].join('\n');
...
async function loadConsole(matchId?: string | null) {
  ...
  return { matches, selectedMatchId, match: selectedMatch, events, latestSnapshot: publicSnapshot(latest), snapshotHistory: history.map(publicSnapshot), existingDigest: digests[0] || null };
}
...
async function saveManualSnapshot(body: any) {
  ...
  const rawData = { sourceName: textOrNull(body.sourceName) || provider, sourceUrl: textOrNull(body.sourceUrl), reviewNote: textOrNull(body.reviewNote), xg: { home: numberOrNull(body.xgHome, 0), away: numberOrNull(body.xgAway, 0) }, savedBy: 'match-data-console' };
  await prisma.matchStatsSnapshot.create({
    data: {
      id: randomUUID(), matchId, provider, providerMatchId,
      minute: integerOrNull(body.minute, 0, 130), homePossession: integerOrNull(body.homePossession, 0, 100), awayPossession: integerOrNull(body.awayPossession, 0, 100),
      homeAttacks: integerOrNull(body.homeAttacks, 0), awayAttacks: integerOrNull(body.awayAttacks, 0), homeDangerousAttacks: integerOrNull(body.homeDangerousAttacks, 0), awayDangerousAttacks: integerOrNull(body.awayDangerousAttacks, 0),
      homeShots: integerOrNull(body.homeShots, 0), awayShots: integerOrNull(body.awayShots, 0), homeShotsOnTarget: integerOrNull(body.homeShotsOnTarget, 0), awayShotsOnTarget: integerOrNull(body.awayShotsOnTarget, 0),
      homeShotsOffTarget: integerOrNull(body.homeShotsOffTarget, 0), awayShotsOffTarget: integerOrNull(body.awayShotsOffTarget, 0), homeCorners: integerOrNull(body.homeCorners, 0), awayCorners: integerOrNull(body.awayCorners, 0),
      homeYellowCards: integerOrNull(body.homeYellowCards, 0), awayYellowCards: integerOrNull(body.awayYellowCards, 0), homeRedCards: integerOrNull(body.homeRedCards, 0), awayRedCards: integerOrNull(body.awayRedCards, 0),
      homeScore: integerOrNull(body.homeScore, 0), awayScore: integerOrNull(body.awayScore, 0), rawData,
    },
  });
