...
async function runSingleMode(req: Request, mode: TheStatsExtrasEndpointMode) {
  ...
  for (let index = 0; index < selected.length; index += 1) {
    ...
    try {
      ...
      results.push({ matchId: match.id, match: matchLabel(match), matchDate: match.matchDate, mode, ok: Boolean(result?.ok), saved: Boolean(result?.saved), snapshotId: result?.snapshotId || null, resolvedProviderMatchId: result?.resolvedProviderMatchId || null, resolvedBy: result?.resolvedBy || null, providerBlocked: blocked, rateLimited: Boolean(result?.rateLimited), counts: result?.counts || null, endpointsOk: result?.endpointsOk || [], endpointsFailed: result?.endpointsFailed || [], error: result?.error || null });
  ...
  return { mode, dryRun, saveRequested: save, skipExisting, limit, candidateWindow, selected: selected.length, processed: results.length, saved: results.filter((item: any) => item.saved).length, successful: results.filter((item: any) => item.ok).length, failed: results.filter((item: any) => !item.ok).length, rateLimited, next: lastProcessed?.matchDate ? { before: order === 'desc' ? lastProcessed.matchDate : undefined, after: order === 'asc' ? lastProcessed.matchDate : undefined, order } : null, advice: rateLimited ? '429/rate limit detected. Stop now and retry later with limit=1 and higher matchDelayMs/endpointDelayMs.' : selected.length === 0 ? 'No eligible finished matches found for this mode. Try another phase or set skipExisting=false for verification.' : 'Safe batch complete. Run the same URL again to continue, or move to the next mode after this mode finishes.', results };
}
