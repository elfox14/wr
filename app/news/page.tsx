...
import { BookOpen, Calendar, Clock, Filter, Newspaper, Sparkles, TrendingUp } from 'lucide-react';
...
export const metadata: Metadata = {
  ...
  description: 'متابعة حية وشاملة لآخر أخبار مباريات كأس العالم، الإصابات، تحليلات المنتخبات واللاعبين، وتحديثات بورصة المونديال الافتراضية.',
  keywords: ['أخبار كأس العالم', 'تحليل مباريات المونديال', 'إصابات كأس العالم', 'بورصة المونديال', 'تحليلات كروية'],
};
...
export default async function NewsPage({ searchParams }: NewsPageProps) {
  ...
  // Segment Hero story (most important or latest) and list stories
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white px-4 py-8 sm:px-6 lg:px-8" dir="rtl">
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        <section className="rounded-[2.5rem] border border-white/10 bg-[radial-gradient(circle_at_top_right,rgba(15,240,252,0.15),transparent_35%),radial-gradient(circle_at_bottom_left,rgba(255,215,0,0.1),transparent_30%),linear-gradient(135deg,rgba(255,255,255,0.05),rgba(255,255,255,0.01))] p-6 shadow-anti-gravity md:p-8">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <h1 className="text-3xl font-black md:text-5xl tracking-tight text-white flex items-center gap-3">
              ...
              <p className="mt-3 max-w-3xl text-sm font-bold leading-7 text-gray-400">
                شاهد كواليس المونديال، تحليلات تكتيكية للمباريات واللاعبين، رصد صحفي دولي، وتقارير حصرية تؤثر على أسعار أصول المنتخبات في السوق الافتراضي.
              </p>
            </div>
          </div>
        </section>

        {/* Category Filters (SEO-friendly links) */}
            ...
            <div className="grid gap-6 p-6 md:grid-cols-2 md:p-8 lg:gap-10">
              <div className="flex flex-col justify-between space-y-4">
                <div className="space-y-3">
                  ...
                  <h2 className="text-2xl font-black leading-tight text-white group-hover:text-[#0FF0FC] transition-colors md:text-3.5xl">
                ...
                <div className="flex items-center justify-between border-t border-white/5 pt-4">
                  ...
                  <Link
                    ...
                    className="inline-flex items-center gap-2 rounded-xl bg-white/10 px-4 py-2.5 text-xs font-black text-white hover:bg-[#0FF0FC] hover:text-black transition-all"
                  >
              ...
              {/* Decorative Visual Box matching site theme */}
              <div className="relative flex min-h-[200px] items-center justify-center rounded-2xl bg-[radial-gradient(circle_at_center,rgba(15,240,252,0.12),transparent_70%),rgba(0,0,0,0.4)] border border-white/5 md:min-h-full">
                <div className="text-center p-6 space-y-2">
                  <Newspaper size={48} className="mx-auto text-[#0FF0FC] opacity-40 animate-pulse" />
                  <p className="text-xs font-black text-[#0FF0FC] tracking-widest">MC PRIME ANALYSIS</p>
                  <p className="text-[10px] font-bold text-gray-500">الأهمية المقدرة: %{heroItem.importance}</p>
                </div>
              </div>
            </div>
          </article>
          ...
          <section className="space-y-6">
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {listItems.map((item) => (
                <article
                  key={item.id}
                  className="flex flex-col justify-between rounded-2xl border border-white/8 bg-white/[0.02] p-5 transition-all hover:border-[#FFD700]/30 hover:bg-white/[0.04]"
                >
                  <div className="space-y-3">
                    ...
                    <h3 className="line-clamp-2 font-black leading-7 text-white hover:text-[#FFD700] transition-colors">
                  ...
                  <div className="mt-4 flex items-center justify-between border-t border-white/5 pt-3 text-xs font-bold text-gray-500">
                    ...
                    <Link
                      href={`/news/${item.id}`}
                      className="text-[#0FF0FC] hover:underline flex items-center gap-1"
                    >
                      التفاصيل ←
                    </Link>
                  </div>
        ...
        {/* Virtual Stock Market Integration Box */}
          ...
          <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
            <div className="space-y-2">
              ...
              <p className="max-w-3xl text-sm font-bold text-gray-400 leading-7">
                تذكر أن التحليلات الكروية وأخبار الإصابات والغيابات في هذا القسم ترتبط ارتباطاً وثيقاً بقوة منتخباتكم ولاعبيكم، مما ينعكس مباشرة على حركة البيع والشراء في البورصة الرياضية الافتراضية.
              </p>
            </div>
            <Link
              href="/market"
