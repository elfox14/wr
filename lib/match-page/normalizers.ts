...
export function normalizeGroupKey(value?: string | null) {
  ...
  const cleaned = raw.replace(/^group[_\s-]*/i, '').replace(/^المجموعة\s*/i, '').replace(/[_-]+/g, ' ').trim().toUpperCase();
  if (!cleaned || BAD_GROUP_KEYS.includes(cleaned)) return null;
  return cleaned;
}
...
export function stageLabel(stage?: string | null, groupPhase?: string | null) {
  ...
  if (!value || BAD_GROUP_KEYS.includes(normalized) || normalized === 'GROUP') return 'مرحلة المجموعات';
...
export function buildSourceList(sources: any[]): MatchSourceView[] {
  const rows = new Map<string, MatchSourceView>();
  const sorted = [...(sources || [])].sort((a, b) => providerPriority(a) - providerPriority(b) || new Date(b?.capturedAt || 0).getTime() - new Date(a?.capturedAt || 0).getTime());

  sorted.forEach((snapshot, index) => {
    const key = providerKey(snapshot);
    if (!snapshot || rows.has(key)) return;
    const name = providerName(snapshot) || String(snapshot?.provider || 'مصدر غير معروف');
    const rawProvider = String(snapshot?.provider || '').trim();
    const capturedAt = snapshot?.capturedAt ? new Date(snapshot.capturedAt).toISOString() : null;
    rows.set(key, {
      key,
      name,
...
export function rawStats(snapshot: any) {
  ...
  const normalizedStats = asObject(data.normalized?.liveStats?.stats);
  return { ...asObject(nested.providerStats), ...asObject(nested.stats), ...asObject(liveStatsPayload.providerStats), ...asObject(liveStatsPayload.stats), ...asObject(liveStatsPayload.overview), ...normalizedStats, ...asObject(data.providerStats), ...asObject(data.stats) };
}
...
function statPair(snapshot: any, homeKey: string, awayKey: string, statKey: string): Pair {
  ...
  const home = aliases.map((alias) => toNumber(stats[homeKey] ?? stats[`home_${alias}`] ?? stats[`${alias}_home`] ?? stats[`${alias}Home`])).find((v) => v !== null) ?? null;
  const away = aliases.map((alias) => toNumber(stats[awayKey] ?? stats[`away_${alias}`] ?? stats[`${alias}_away`] ?? stats[`${alias}Away`])).find((v) => v !== null) ?? null;
...
export function buildStatMetric(sources: any[], key: string, label: string, homeKey: string, awayKey: string, suffix = ''): MatchStatMetric {
  const pair = sources.map((snapshot) => statPair(snapshot, homeKey, awayKey, key)).find(Boolean) as Pair;
  return { key, label, home: pair?.home ?? null, away: pair?.away ?? null, suffix, source: pair?.source || '', available: pair !== null };
}

export function metricDefinitions(): Array<[string, string, string, string, string?]> {
  return [
...
function snapshotMinute(snapshot: any) {
  ...
  return toNumber(snapshot?.minute ?? data.minute ?? flashMeta.elapsed ?? flashMeta.minute ?? nestedFlashMeta.elapsed ?? meta.elapsed ?? meta.minute);
}
...
function statusFromProviderValue(value: unknown, minute: number | null) {
  ...
  if (['FIRST_HALF', 'FIRST', '1ST_HALF'].includes(raw)) return '1H';
  if (['SECOND_HALF', 'SECOND', '2ND_HALF'].includes(raw)) return '2H';
...
function statusCandidate(snapshot: any): StatusCandidate | null {
  ...
  return { status, minute, priority: providerPriority(snapshot), capturedAt: snapshot?.capturedAt ? new Date(snapshot.capturedAt).getTime() : 0, sourceKey: providerKey(snapshot) };
}
...
export function statusFromSnapshots(sources: any[]) {
  ...
  const flash = candidates.filter((candidate) => candidate.sourceKey === 'isports-flash').sort((a, b) => b.capturedAt - a.capturedAt)[0];
...
export function scoreFromSnapshot(snapshot: any): MatchScore | null {
  ...
  const meta = asObject(data.meta);
  const flashMeta = asObject(data.flashMeta);
  const finalScore = asObject(data.normalized?.matchInfo?.finalScore);
  const home = toNumber(snapshot.homeScore ?? data.homeScore ?? data.home_goals ?? finalScore.home ?? flashMeta.homeScore ?? meta.home_goals ?? counts.homeScore);
  const away = toNumber(snapshot.awayScore ?? data.awayScore ?? data.away_goals ?? finalScore.away ?? flashMeta.awayScore ?? meta.away_goals ?? counts.awayScore);
...
function elapsedMinuteFromKickoff(match: any) {
  ...
  return Math.max(1, Math.min(FINAL_MINUTE_FALLBACK + 10, elapsed));
}
...
export function buildStatusView(match: any, sources: any[]): MatchStatusView {
  ...
  const startMs = new Date(match.matchDate || '').getTime();
  if (Number.isFinite(startMs) && Date.now() > startMs + 5 * 60_000 && kind === 'scheduled') return { raw, kind: 'delayed', label: 'بانتظار تأكيد البداية', shortLabel: 'تأخر البدء', minute: null, isLive: false, isFinished: false, isScheduled: false };
...
export function buildEventView(event: any): MatchEventView {
  return { id: event.id, minute: event.minute ?? null, minuteLabel: eventMinuteLabel(event), type: event.type || 'note', icon: eventIcon(event.type), teamId: event.teamId || null, playerName: event.playerName || null, detail: event.detail || 'حدث مباراة', sourceName: event.sourceName || null, sourceUrl: event.sourceUrl || null };
}
