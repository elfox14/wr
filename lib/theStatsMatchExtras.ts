...
function compactEvent(row: any, sourcePath: string) {
  ...
  return {
    type: str(row?.type, row?.event_type, row?.incident_type, row?.name) || rawType || 'event',
    normalizedType: rawType,
    minute,
    teamName: str(team?.name, row?.team_name, row?.teamName),
    playerName: str(player?.name, row?.player_name, row?.playerName, row?.scorer?.name),
    playerIn: str(playerIn?.name, row?.player_in_name, row?.playerInName, row?.in_name),
    playerOut: str(playerOut?.name, row?.player_out_name, row?.playerOutName, row?.out_name),
...
function compactPlayerStat(row: any, sourcePath: string) {
  ...
  return {
    id: str(player?.id, row?.player_id, row?.id),
    name: str(player?.name, player?.full_name, row?.name, row?.playerName, row?.display_name),
    teamName: str(team?.name, row?.team_name, row?.teamName),
    position: str(player?.position, row?.position),
    rating: n(stats?.rating ?? row?.rating),
    minutes: n(stats?.minutes ?? stats?.minutes_played ?? row?.minutes),
    shots: n(stats?.shots ?? stats?.total_shots),
    shotsOnTarget: n(stats?.shots_on_target ?? stats?.shotsOnTarget),
    goals: n(stats?.goals),
    assists: n(stats?.assists),
    passes: n(stats?.passes ?? stats?.total_passes),
    keyPasses: n(stats?.key_passes ?? stats?.keyPasses),
    crosses: n(stats?.crosses),
    tackles: n(stats?.tackles),
    interceptions: n(stats?.interceptions),
    clearances: n(stats?.clearances),
    blocks: n(stats?.blocks),
    duels: n(stats?.duels ?? stats?.duels_total),
    foulsCommitted: n(stats?.fouls_committed ?? stats?.foulsCommitted),
    foulsWon: n(stats?.fouls_won ?? stats?.foulsWon),
    saves: n(stats?.saves ?? stats?.goalkeeper_saves),
    goalsPrevented: n(stats?.goals_prevented ?? stats?.goalsPrevented ?? stats?.psxg_minus_goals),
    savesInsideBox: n(stats?.saves_inside_box ?? stats?.savesInsideBox),
    sourcePath,
  };
}
...
function eventBuckets(events: any[]) {
  return {
    substitutions: events.filter((event) => event.normalizedType.includes('sub')),
    cards: events.filter((event) => event.normalizedType.includes('card') || event.normalizedType.includes('yellow') || event.normalizedType.includes('red')),
    penalties: events.filter((event) => event.normalizedType.includes('penalty')),
    var: events.filter((event) => event.normalizedType.includes('var')),
  };
}
...
function matchInfoFromPayloads(payloads: Record<string, any>) {
  ...
  const weather = fixture?.weather || matchPayload?.weather;
  return {
    venue: str(venue?.name, venue?.stadium, venue, fixture?.venue_name, matchPayload?.venue_name),
    city: str(venue?.city, fixture?.city, matchPayload?.city),
    country: str(venue?.country, fixture?.country, matchPayload?.country),
    referee: str(referee?.name, referee, fixture?.referee_name, matchPayload?.referee_name),
    attendance: n(fixture?.attendance ?? matchPayload?.attendance),
    weather,
    finalScore: {
      home: n(fixture?.home_goals ?? matchPayload?.home_goals ?? matchPayload?.meta?.home_goals),
      away: n(fixture?.away_goals ?? matchPayload?.away_goals ?? matchPayload?.meta?.away_goals),
    },
    manOfTheMatch: str(matchPayload?.man_of_the_match?.name, matchPayload?.manOfTheMatch?.name, matchPayload?.motm?.name, matchPayload?.player_of_the_match?.name),
  };
}
...
function compactStandings(payload: any) {
  ...
  return rows.map((row: any) => ({
    rank: n(row?.rank ?? row?.position),
    teamName: str(row?.team?.name, row?.team_name, row?.name),
    played: n(row?.played ?? row?.matches_played),
    won: n(row?.won ?? row?.wins),
    drawn: n(row?.drawn ?? row?.draws),
    lost: n(row?.lost ?? row?.losses),
    goalsFor: n(row?.goals_for ?? row?.goalsFor ?? row?.gf),
    goalsAgainst: n(row?.goals_against ?? row?.goalsAgainst ?? row?.ga),
    goalDifference: n(row?.goal_difference ?? row?.goalDifference ?? row?.gd),
    points: n(row?.points ?? row?.pts),
  }));
}
...
async function fetchEndpoint(endpoint: any, providerId: string, timeoutMs: number) {
  ...
  try {
    ...
    return { key: endpoint.key, path, ok: true, payload, keySummary: keySummary(payload) };
  } catch (error: any) {
    return { key: endpoint.key, path, ok: false, error: safeTheStatsApiError(error), keySummary: null };
  }
}
...
function normalizeExtras(endpointResults: Record<string, any>) {
  ...
  const shotRows = [
    ...listFrom(endpointResults.shotmap?.payload, ['shotmap', 'shots', 'events', 'items', 'results']),
    ...listFrom(endpointResults.shots?.payload, ['shots', 'shotmap', 'events', 'items', 'results']),
  ];
  const shotmap = shotRows.map((row) => compactShot(row, 'shotmap/shots'));
  const playerRows = [
    ...listFrom(endpointResults.playerStats?.payload, ['players', 'player_stats', 'statistics', 'items', 'results']),
    ...listFrom(endpointResults.playerStatistics?.payload, ['players', 'player_stats', 'statistics', 'items', 'results']),
    ...listFrom(endpointResults.players?.payload, ['players', 'items', 'results']),
  ];
  const playerStats = playerRows.map((row) => compactPlayerStat(row, 'player-stats'));
  return {
    matchInfo: matchInfoFromPayloads(endpointResults),
    shotmap,
    eventsDetailed: { all: events, ...eventBuckets(events) },
    playerStats,
    goalkeeperStats: playerStats.filter((player) => player.saves !== null || player.goalsPrevented !== null || player.savesInsideBox !== null),
    standings: compactStandings(endpointResults.standings?.payload),
  };
}
...
export async function collectTheStatsMatchExtras(match: any, options: { dryRun: boolean; save: boolean; timeoutMs: number; includeRaw: boolean; query: Record<string, string | number> }) {
  ...
  const results = await Promise.all(ENDPOINTS.map((endpoint) => fetchEndpoint(endpoint, resolved.id!, options.timeoutMs)));
  ...
  if (!options.dryRun && options.save) {
    const snapshot = await prisma.matchStatsSnapshot.create({ data: {
      ...
      rawData: {
        ...
        endpoints: results.map((result) => ({ key: result.key, path: result.path, ok: result.ok, error: result.ok ? null : result.error, keySummary: result.keySummary })),
        normalized,
        raw: options.includeRaw ? Object.fromEntries(results.filter((r) => r.ok).map((r) => [r.key, r.payload])) : undefined,
      },
    }, select: { id: true } });
  ...
  return {
    ...
    resolvedProviderMatchId: resolved.id,
    resolvedBy: resolved.by,
    endpointsOk: results.filter((result) => result.ok).map((result) => result.key),
    endpointsFailed: results.filter((result) => !result.ok).map((result) => ({ key: result.key, status: result.error?.status, code: result.error?.code, message: result.error?.message })),
    counts: {
      shots: normalized.shotmap.length,
      detailedEvents: normalized.eventsDetailed.all.length,
      substitutions: normalized.eventsDetailed.substitutions.length,
      cards: normalized.eventsDetailed.cards.length,
      penalties: normalized.eventsDetailed.penalties.length,
      var: normalized.eventsDetailed.var.length,
      playerStats: normalized.playerStats.length,
      goalkeeperStats: normalized.goalkeeperStats.length,
      standings: normalized.standings.length,
    },
    matchInfo: normalized.matchInfo,
    saved: Boolean(snapshotId),
    snapshotId,
    debug: options.includeRaw ? { endpoints: endpointResults, normalized } : { endpointSummaries: results.map((r) => ({ key: r.key, path: r.path, ok: r.ok, error: r.ok ? null : r.error, keySummary: r.keySummary })), normalizedPreview: normalized },
  };
}
...
export function defaultTheStatsQuery(params: URLSearchParams) {
  return {
    ...
    per_page: Math.max(1, Math.min(100, Number(params.get('providerMatchesPerPage') || 100) || 100)),
  };
}
