...
async function findTeamByCodes(codes: string[]) {
...
async function main() {
  ...
  for (const report of reports) {
    const team = await findTeamByCodes(report.teamCodes);
    if (!team) {
      skipped++;
      console.warn(`Skipping report: no team found for codes ${report.teamCodes.join(', ')}`);
      continue;
    }
  ...
  console.log(`Team intelligence reports seed completed. Created: ${created}. Skipped: ${skipped}.`);
}
